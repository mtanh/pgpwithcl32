...
BOOLEAN sanityCheckEnvelope( const ENVELOPE_INFO *envelopeInfoPtr )
	...
	if( envelopeInfoPtr->type < CRYPT_FORMAT_NONE || \
		...
		{
		DEBUG_PRINT(( "sanityCheckEnvelope: General info" ));
		return( FALSE );
		}
	...
	if( envelopeInfoPtr->cryptActionSize != CRYPT_UNUSED && \
		...
		{
		DEBUG_PRINT(( "sanityCheckEnvelope: Crypt actions" ));
		return( FALSE );
		}
	if( envelopeInfoPtr->signActionSize != CRYPT_UNUSED && \
		...
		{
		DEBUG_PRINT(( "sanityCheckEnvelope: Sign actions" ));
		return( FALSE );
		}
	if( envelopeInfoPtr->extraDataSize < 0 || \
		envelopeInfoPtr->extraDataSize >= MAX_INTLENGTH_SHORT )
		{
		DEBUG_PRINT(( "sanityCheckEnvelope: Extra data" ));
		return( FALSE );
		}
	...
	if( envelopeInfoPtr->bufSize < 0 || \
		...
		{
		DEBUG_PRINT(( "sanityCheckEnvelope: Buffer info" ));
		return( FALSE );
		}
	if( envelopeInfoPtr->auxBuffer == NULL )
		...
		if( envelopeInfoPtr->auxBufPos != 0 || \
			...
			{
			DEBUG_PRINT(( "sanityCheckEnvelope: Spurious auxiliary buffer" ));
			return( FALSE );
			}
		}
	else
		...
		if( envelopeInfoPtr->auxBufSize <= 0 || \
			...
			{
			DEBUG_PRINT(( "sanityCheckEnvelope: Auxiliary buffer info" ));
			return( FALSE );
			}
		}
	if( envelopeInfoPtr->partialBufPos < 0 || \
		...
		{
		DEBUG_PRINT(( "sanityCheckEnvelope: Partial buffer info" ));
		return( FALSE );
		}
	if( envelopeInfoPtr->blockBufferPos < 0 || \
		...
		{
		DEBUG_PRINT(( "sanityCheckEnvelope: Block buffer info" ));
		return( FALSE );
		}
	if( envelopeInfoPtr->oobEventCount < 0 || \
		...
		{
		DEBUG_PRINT(( "sanityCheckEnvelope: OOB info" ));
		return( FALSE );
		}
	...
	if( envelopeInfoPtr->state < ENVELOPE_STATE_NONE || \
		...
		{
		DEBUG_PRINT(( "sanityCheckEnvelope: State" ));
		return( FALSE );
		}
#ifdef USE_PGP
	if( envelopeInfoPtr->pgpDeenvState < PGP_DEENVSTATE_NONE || \
		...
		{
		DEBUG_PRINT(( "sanityCheckEnvelope: PGP state" ));
		return( FALSE );
		}
#endif /* USE_PGP */
	...
	if( envelopeInfoPtr->payloadSize != CRYPT_UNUSED && \
		...
		{
		DEBUG_PRINT(( "sanityCheckEnvelope: Payload size" ));
		return( FALSE );
		}
	if( envelopeInfoPtr->segmentSize < 0 || \
		...
		{
		DEBUG_PRINT(( "sanityCheckEnvelope: Segment size" ));
		return( FALSE );
		}
	...
	if( envelopeInfoPtr->segmentStart < 0 || \
		...
		{
		DEBUG_PRINT(( "sanityCheckEnvelope: Segment info" ));
		return( FALSE );
		}
	...
	if( !isHandleRangeValid( envelopeInfoPtr->objectHandle ) || \
		...
		{
		DEBUG_PRINT(( "sanityCheckEnvelope: Object handles" ));
		return( FALSE );
		}
...
static int envelopeMessageFunction( INOUT TYPECAST( ENVELOPE_INFO * ) \
	...
	if( message == MESSAGE_DESTROY )
		...
		if( DATAPTR_GET( envelopeInfoPtr->contentList ) != NULL )
