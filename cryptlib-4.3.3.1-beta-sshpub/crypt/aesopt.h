...
/* In theory this is possible for gcc 4.7 and greater, however trying to use
   it with gcc 4.9 just gives endless undefined references to all the SSE 
   intrinsics, so instead of the following we only enable it for gcc 5 - pcg */

#if 0

#if (defined( _WIN64 ) && defined( _MSC_VER )) \
 || (defined( __GNUC__ ) && \
	( ( __GNUC__ == 4 && __GNUC_MINOR__ >= 7 ) || ( __GNUC__ >= 5 ) ) /* pcg */ && \
	defined( __x86_64__ )) \
...
#if (defined( _WIN64 ) && defined( _MSC_VER )) \
 || (defined( __GNUC__ ) && ( __GNUC__ >= 5 ) /* pcg */ && \
	defined( __x86_64__ )) \
 && !(defined( INTEL_AES_POSSIBLE ))
#  define INTEL_AES_POSSIBLE
#endif
