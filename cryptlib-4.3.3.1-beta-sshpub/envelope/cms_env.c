...
static BOOLEAN sanityCheckEnvCMSEnv( const ENVELOPE_INFO *envelopeInfoPtr )
	...
	if( !sanityCheckEnvelope( envelopeInfoPtr ) )
		{
		DEBUG_PRINT(( "sanityCheckEnvCMSEnv: Envelope check" ));
		return( FALSE );
		}
	...
	if( envelopeInfoPtr->type != CRYPT_FORMAT_CRYPTLIB && \
		...
		{
		DEBUG_PRINT(( "sanityCheckEnvCMSEnv: Type" ));
		return( FALSE );
		}
	if( envelopeInfoPtr->flags & ENVELOPE_ISDEENVELOPE )
		{
		DEBUG_PRINT(( "sanityCheckEnvCMSEnv: General info" ));
		return( FALSE );
		}
	...
	if( envelopeInfoPtr->buffer == NULL || \
		...
		{
		DEBUG_PRINT(( "sanityCheckEnvCMSEnv: Buffer" ));
		return( FALSE );
		}
...
static const OID_INFO FAR_DATA contentOIDs[] = {
...
static int writeSignedDataHeader( INOUT STREAM *stream,
	...
	ENSURES( hashActionSize > 0 && hashActionSize < MAX_INTLENGTH_SHORT );
...
static int writeEnvelopeHeader( INOUT ENVELOPE_INFO *envelopeInfoPtr )
	...
	if( ( envelopeInfoPtr->usage != ACTION_CRYPT && \
		  envelopeInfoPtr->usage != ACTION_MAC ) || preActionListPtr == NULL )
		{
	...
	actionListPtr = findActionEx( preActionListPtr, ACTION_KEYEXCHANGE_PKC );
	if( actionListPtr == NULL )
		actionListPtr = findActionEx( preActionListPtr, ACTION_KEYEXCHANGE );
...
static int writeCertchainTrailer( INOUT ENVELOPE_INFO *envelopeInfoPtr )
	...
	lastActionPtr = DATAPTR_GET( envelopeInfoPtr->postActionList );
...
static int emitPreamble( INOUT ENVELOPE_INFO *envelopeInfoPtr )
	...
	if( envelopeInfoPtr->envState == ENVSTATE_ENCRINFO )
		...
		REQUIRES( actionListPtr != NULL );
		REQUIRES( contentOID != NULL );
		REQUIRES( dataLeft >= 0 && dataLeft < MAX_INTLENGTH_SHORT );
