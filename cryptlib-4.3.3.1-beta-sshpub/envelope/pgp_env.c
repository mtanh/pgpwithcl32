...
static BOOLEAN sanityCheckPGPEnv( const ENVELOPE_INFO *envelopeInfoPtr )
	...
	if( !sanityCheckEnvelope( envelopeInfoPtr ) )
		{
		DEBUG_PRINT(( "sanityCheckPGPEnv: Envelope check" ));
		return( FALSE );
		}
	...
	if( envelopeInfoPtr->type != CRYPT_FORMAT_PGP || \
		...
		{
		DEBUG_PRINT(( "sanityCheckPGPEnv: General info" ));
		return( FALSE );
		}
	...
	if( envelopeInfoPtr->buffer == NULL || \
		...
		{
		DEBUG_PRINT(( "sanityCheckPGPEnv: Buffer" ));
		return( FALSE );
		}
	...
	if( envelopeInfoPtr->auxBuffer != NULL || \
		...
		{
		DEBUG_PRINT(( "sanityCheckPGPEnv: Spurious auxiliary buffer" ));
		return( FALSE );
		}
...
static int writeHeaderPacket( INOUT ENVELOPE_INFO *envelopeInfoPtr )
	...
	if( envelopeInfoPtr->usage == ACTION_CRYPT )
		...
		lastActionPtr = findActionEx( preActionListPtr, 
...
static int preEnvelopeEncrypt( INOUT ENVELOPE_INFO *envelopeInfoPtr )
	...
	REQUIRES( findActionEx( DATAPTR_GET( envelopeInfoPtr->preActionList ), \
	...
	LOOP_MED( actionListCursor = \
					findActionEx( DATAPTR_GET( envelopeInfoPtr->preActionList ),
								  ACTION_KEYEXCHANGE_PKC ), 
		...
		actionListCursor->associatedAction = actionListPtr;
...
static int preEnvelopeSign( const ENVELOPE_INFO *envelopeInfoPtr )
	...
	REQUIRES( postActionListPtr->associatedAction != NULL );
	...
	return( iCryptCreateSignature( NULL, 0, &postActionListPtr->encodedSize, 
						...
						postActionListPtr->associatedAction->iCryptHandle,
						&sigParams ) );
	}
