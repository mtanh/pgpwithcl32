...
static const CMS_CONTENT_INFO FAR_DATA oidInfoSignedData = { 0, 3 };
static const CMS_CONTENT_INFO FAR_DATA oidInfoEnvelopedData = { 0, 4 };
static const CMS_CONTENT_INFO FAR_DATA oidInfoEncryptedData = { 0, 2 };
static const CMS_CONTENT_INFO FAR_DATA oidInfoCompressedData = { 0, 0 };
static const CMS_CONTENT_INFO FAR_DATA oidInfoAuthData = { 0, 0 };
static const CMS_CONTENT_INFO FAR_DATA oidInfoAuthEnvData = { 0, 0 };

static const OID_INFO FAR_DATA envelopeOIDinfo[] = {
...
static const OID_INFO FAR_DATA nestedContentOIDinfo[] = {
...
static BOOLEAN sanityCheckEnvCMSDenv( const ENVELOPE_INFO *envelopeInfoPtr )
	...
	if( !sanityCheckEnvelope( envelopeInfoPtr ) )
		{
		DEBUG_PRINT(( "sanityCheckEnvCMSDenv: Envelope check" ));
		return( FALSE );
		}
	...
	if( !( envelopeInfoPtr->flags & ENVELOPE_ISDEENVELOPE ) )
		{
		DEBUG_PRINT(( "sanityCheckEnvCMSDenv: General info" ));
		return( FALSE );
		}
	...
	if( envelopeInfoPtr->buffer == NULL || \
		...
		{
		DEBUG_PRINT(( "sanityCheckEnvCMSDenv: Buffer" ));
		return( FALSE );
		}
...
static int addContentListItem( INOUT ENVELOPE_INFO *envelopeInfoPtr,
	...
	const CONTENT_LIST *contentListPtr = \
					FNPTR_GET( envelopeInfoPtr->contentList );
...
static int processPreamble( INOUT ENVELOPE_INFO *envelopeInfoPtr )
	...
	/* Keep consuming information until we either run out of input or reach 
	   the data payload.  The limit of 100 header items would never occur in 
	   any normal usage but in theory it's possible to generate S/MIME 
	   messages with large numbers of recipients for mailing lists so we set
	   the limit at 100 */
	LOOP_LARGE( noHeaderItems = 0, 
		...
		switch( state )
			...
			case DEENVSTATE_ENCRCONTENT:
				...
				streamPos = stell( &stream );
				state = DEENVSTATE_DATA;
				if( DATAPTR_GET( envelopeInfoPtr->actionList ) == NULL )
					{
	...
	if( noHeaderItems >= 32 )
		...
		setErrorString( ENVELOPE_ERRINFO, 
						"Encountered excessive number of envelope header "
						"items", 53 );
		return( CRYPT_ERROR_BADDATA );
		}
...
static int processPostamble( INOUT ENVELOPE_INFO *envelopeInfoPtr,
	...
	LOOP_MED( noTrailerItems = 0,
			  state != DEENVSTATE_DONE && noTrailerItems < 32,
			  noTrailerItems++ )
		{
	...
	if( failedMAC )
		...
		setErrorString( ENVELOPE_ERRINFO, 
						"MAC value doesn't match calculated MAC", 38 );
