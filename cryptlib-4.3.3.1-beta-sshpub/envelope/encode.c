...
static BOOLEAN sanityCheckEnvEncode( const ENVELOPE_INFO *envelopeInfoPtr )
	...
	if( envelopeInfoPtr->buffer == NULL || \
		...
		{
		DEBUG_PRINT(( "sanityCheckEnvEncode: Buffer" ));
		return( FALSE );
		}
	...
	if( envelopeInfoPtr->blockSize > 0 && \
		...
		{
		DEBUG_PRINT(( "sanityCheckEnvEncode: Block buffer" ));
		return( FALSE );
		}
	...
	if( envelopeInfoPtr->partialBufPos != 0 || \
		...
		{
		DEBUG_PRINT(( "sanityCheckEnvEncode: Spurious OOB/partial buffer" ));
		return( FALSE );
		}
	...
	if( envelopeInfoPtr->segmentStart < 0 || \
		...
		{
		DEBUG_PRINT(( "sanityCheckEnvEncode: Segment start" ));
		return( FALSE );
		}
	if( envelopeInfoPtr->segmentDataStart < 0 || \
		...
		{
		DEBUG_PRINT(( "sanityCheckEnvEncode: Segment data start" ));
		return( FALSE );
		}
	...
	if( envelopeInfoPtr->payloadSize != CRYPT_UNUSED )
		...
		if( envelopeInfoPtr->segmentStart != \
			...
			{
			DEBUG_PRINT(( "sanityCheckEnvEncode: Non-segmented encoding" ));
			return( FALSE );
			}
	...
	if( envelopeInfoPtr->segmentStart >= envelopeInfoPtr->bufPos )
		{
		DEBUG_PRINT(( "sanityCheckEnvEncode: Segmented encoding" ));
		return( FALSE );
		}
	...
	if( envelopeInfoPtr->segmentStart >= envelopeInfoPtr->segmentDataStart )
		{
		DEBUG_PRINT(( "sanityCheckEnvEncode: Segmented encoding nesting" ));
		return( FALSE );
		}
