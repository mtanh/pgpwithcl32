...
static int checkPgpUsage( INOUT ENVELOPE_INFO *envelopeInfoPtr,
	...
	if( findActionEx( DATAPTR_GET( envelopeInfoPtr->preActionList ),
...
static int checkMissingInfo( INOUT ENVELOPE_INFO *envelopeInfoPtr,
	...
	switch( envelopeInfoPtr->usage )
		...
		case ACTION_MAC:
			...
			if( findActionEx( preActionListPtr, ACTION_KEYEXCHANGE_PKC ) == NULL && \
				findActionEx( preActionListPtr, ACTION_KEYEXCHANGE ) == NULL )
				{
		...
		case ACTION_SIGN:
			/* If it's a signing envelope there must be a signature key
			   present */
			if( findActionEx( postActionListPtr, ACTION_SIGN ) == NULL )
				{
...
static int addPgpPasswordInfo( ENVELOPE_INFO *envelopeInfoPtr,
	...
	if( envelopeInfoPtr->type == CRYPT_FORMAT_PGP && \
		( findActionEx( preActionListPtr, ACTION_KEYEXCHANGE_PKC ) != NULL || \
...
static int addContextInfo( INOUT ENVELOPE_INFO *envelopeInfoPtr,
	...
	if( actionType == ACTION_HASH && \
		...
		hashActionPtr = findActionEx( actionListPtr, ACTION_HASH );
	...
	if( DATAPTR_GET( envelopeInfoPtr->actionList ) == NULL )
		...
		hashActionPtr = findLastAction( DATAPTR_GET( envelopeInfoPtr->actionList ),
										ACTION_HASH );
		if( hashActionPtr == NULL )
			{
			setErrorInfo( envelopeInfoPtr, CRYPT_ENVINFO_HASH,
	...
	switch( envInfo )
		...
		case CRYPT_ENVINFO_TIMESTAMP:
			...
			ACTION_LIST *actionListPtr = \
				findLastAction( DATAPTR_GET( envelopeInfoPtr->postActionList ),
								ACTION_SIGN );
