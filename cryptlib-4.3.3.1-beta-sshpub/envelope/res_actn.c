...
CHECK_RETVAL_PTR \
ACTION_LIST *findActionEx( IN_OPT const ACTION_LIST *actionListPtr,
...
ACTION_LIST *findAction( const ENVELOPE_INFO *envelopeInfoPtr,
						 IN_ENUM( ACTION ) const ACTION_TYPE actionType )
	{
	assert( isReadPtr( envelopeInfoPtr, sizeof( ENVELOPE_INFO ) ) );

	return( findActionEx( DATAPTR_GET( envelopeInfoPtr->actionList ),
						  actionType ) );
	}

CHECK_RETVAL_PTR \
ACTION_LIST *findLastAction( IN_OPT const ACTION_LIST *actionListPtr,
							 IN_ENUM( ACTION ) const ACTION_TYPE actionType )
	{
	int LOOP_ITERATOR;
	...
	LOOP_MED_CHECKINC( DATAPTR_GET( actionListPtr->next ) != NULL,
					   actionListPtr = DATAPTR_GET( actionListPtr->next ) )
		{
		const ACTION_LIST *actionListNext = DATAPTR_GET( actionListPtr->next );

		REQUIRES_N( actionListNext != NULL );

		if( actionListNext->action != actionType )
			break;
		}
	ENSURES_N( LOOP_BOUND_OK );
...
static int createNewAction( OUT_OPT_PTR_COND ACTION_LIST **newActionPtrPtr,
	...
	switch( actionListType )
		{
		case ACTIONLIST_PREACTION:
			insertSingleListElementSafe( envelopeInfoPtr->preActionList, prevActionPtr, 
										 newItem, ACTION_LIST );
			break;
		
		case ACTIONLIST_ACTION:
			insertSingleListElementSafe( envelopeInfoPtr->actionList, prevActionPtr, 
										 newItem, ACTION_LIST );
			break;
		
		case ACTIONLIST_POSTACTION:
			insertSingleListElementSafe( envelopeInfoPtr->postActionList, prevActionPtr, 
										 newItem, ACTION_LIST );
			break;
...
static int deleteAction( INOUT ENVELOPE_INFO *envelopeInfoPtr,	
	...
	deleteSingleListElementSafe( envelopeInfoPtr->actionList, listPrevPtr, 
		...
		if( envelopeInfoPtr->type == CRYPT_FORMAT_PGP )
			...
			if( actionListPtrNext != NULL )
				...
				if( actionListPtrNext->action != ACTION_HASH || \
					DATAPTR_GET( actionListPtrNext->next ) != NULL )
					return( FALSE );
				}
