...
static const USER_FILE_INFO FAR_DATA primarySOInfo = {
...
static int createUserEntry( OUT_PTR_COND USER_FILE_INFO **userIndexPtrPtr,
	...
	{
	USER_FILE_INFO *userIndexPtr;
	int fileRef, status = CRYPT_OK, LOOP_ITERATOR;
	...
	*userIndexPtrPtr = NULL;
	...
	LOOP_LARGE_CHECK( !cryptStatusError( status ) )
		{
...
BOOLEAN isZeroisePassword( IN_BUFFER( passwordLen ) const char *password,
	...
	REQUIRES_B( passwordLen > 0 && passwordLen < MAX_INTLENGTH_SHORT );
