...
const char *getAlgoName( IN_ALGO const CRYPT_ALGO_TYPE cryptAlgo )
	...
	static const DESCRIPTION_MAP descriptionMap[] = {
		...
		/* Hash algorithms */
		{ CRYPT_ALGO_MD5, "CRYPT_ALGO_MD5" },
		{ CRYPT_ALGO_SHA1, "CRYPT_ALGO_SHA1" },
		{ CRYPT_ALGO_SHA2, "CRYPT_ALGO_SHA2" },
		{ CRYPT_ALGO_SHAng,	"CRYPT_ALGO_SHAng" },
		{ CRYPT_ALGO_NONE, "NONE" }, { CRYPT_ALGO_NONE, "NONE" },
		};
	int i, LOOP_ITERATOR;

	REQUIRES_EXT( isPkcAlgo( cryptAlgo ) || isHashAlgo( cryptAlgo ), 
				  "<<<Unknown>>>" );
