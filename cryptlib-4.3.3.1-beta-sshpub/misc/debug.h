...
#if defined( NDEBUG ) || !defined( USE_ERRMSGS ) || defined( CONFIG_FUZZ ) 
  /* Debugging diagnostics are disabled for release builds, for builds
     without verbose error messages enabled, and for fuzzing builds which 
	 don't do anything with output and for which lots of spew just slows 
