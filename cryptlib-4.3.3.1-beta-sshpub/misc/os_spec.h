...
/* The IAR compiler warns about all manner of pointless stuff, including 
   things that don't have any effect on compiling (e.g. CRLF vs. LF line 
   endings) or are a mandatory part of the C standard (type 1 promoted to
   type 2).  As an added benefit it includes warnings that can never be 
   disabled even if it seems that they can.  We try and disable the more 
   annoying and pointless ones here, but the fact that there's a 
   diag_suppress for a particular warning doesn't mean that it'll actually 
   get suppressed */
...
/* Useful data types.  Newer compilers provide a 'bool' datatype via 
   ...
   for booleans (see int_api.h) means that we can't use a bool because it's
   only defined for the values 0 and 1, a single fault away from each other.
...
/* In 16-bit environments the constant data is large enough that it 
   overflows the (64K) data segment.  Because of this we move as much of it 
   as possible into its own segment with the following define */

#if defined( __WIN16__ )
  #ifdef _MSC_VER
	#define FAR_DATA	__far
  #else
	#define FAR_DATA	far
...
#ifdef __GNUC__
  /* Apple do it differently to everyone else in the universe */
  #if defined( __APPLE__ )
