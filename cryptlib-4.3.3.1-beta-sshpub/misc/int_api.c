...
CHECK_RETVAL_BOOL STDC_NONNULL_ARG( ( 1 ) ) \
BOOLEAN isEmptyData( IN_BUFFER( dataLen ) const BYTE *data,
					 IN_LENGTH_SHORT_Z const int dataLen )
	{
	assert( isReadPtr( data, 8 ) );

	REQUIRES_B( dataLen >= 0 && dataLen < MAX_INTLENGTH_SHORT );
...
static void randomDelay( const int maxDelay )
	...
	ENSURES_V( maxDelay >= 100 && maxDelay <= 1000 );
	...
	if( delayTime < 5 )
		delayTime = 5;
	ENSURES_V( delayTime >= 5 && delayTime <= 1000 );
...
static BOOLEAN testReadLine( IN_BUFFER( dataInLength ) char *dataIn,
	...
	REQUIRES_B( dataOutLength >= 1 && dataOutLength < MAX_INTLENGTH_SHORT );
	REQUIRES( allowContinuation == TRUE || allowContinuation == FALSE );
