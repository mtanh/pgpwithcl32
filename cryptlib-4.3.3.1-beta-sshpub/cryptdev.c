...
BOOLEAN sanityCheckDevice( const DEVICE_INFO *deviceInfoPtr )
	{
	...
	if( deviceInfoPtr->type < CRYPT_DEVICE_NONE || \
		...
		deviceInfoPtr->flags < DEVICE_FLAG_NONE || \
		deviceInfoPtr->flags > DEVICE_FLAG_MAX )
		{
		DEBUG_PRINT(( "sanityCheckDevice: General info" ));
		return( FALSE );
		}
	if( !isEmptyData( deviceInfoPtr->label, deviceInfoPtr->labelLen ) )
		...
		if( deviceInfoPtr->labelLen <= 0 || \
			...
			{
			DEBUG_PRINT(( "sanityCheckDevice: Label" ));
			return( FALSE );
			}
		}
	...
	if( !DATAPTR_ISVALID( deviceInfoPtr->capabilityInfoList ) )
		{
		DEBUG_PRINT(( "sanityCheckDevice: Data pointers" ));
		return( FALSE );
		}
	...
	if( deviceInfoPtr->type == CRYPT_DEVICE_NONE )
		...
		if( deviceInfoPtr->objectHandle != SYSTEM_OBJECT_HANDLE || \
			...
			{
			DEBUG_PRINT(( "sanityCheckDevice: System device handles" ));
			return( FALSE );
			}
		}
	else
		...
		if( !isHandleRangeValid( deviceInfoPtr->objectHandle ) || \
			...
			{
			DEBUG_PRINT(( "sanityCheckDevice: Object handles" ));
			return( FALSE );
			}
		}

	/* Check function and data pointers */
	if( FNPTR_GET( deviceInfoPtr->initFunction ) == NULL || \
		FNPTR_GET( deviceInfoPtr->controlFunction ) == NULL )
		{
		DEBUG_PRINT(( "sanityCheckDevice: General functions" ));
		return( FALSE );
		}
	if( deviceInfoPtr->type != CRYPT_DEVICE_NONE && \
		( FNPTR_GET( deviceInfoPtr->getItemFunction ) == NULL || \
		  FNPTR_GET( deviceInfoPtr->setItemFunction ) == NULL || \
		  FNPTR_GET( deviceInfoPtr->deleteItemFunction ) == NULL || \
		  FNPTR_GET( deviceInfoPtr->getFirstItemFunction ) == NULL || \
		  FNPTR_GET( deviceInfoPtr->getNextItemFunction ) == NULL ) )
		{
		DEBUG_PRINT(( "sanityCheckDevice: Data access functions" ));
		return( FALSE );
		}
	if( deviceInfoPtr->type == CRYPT_DEVICE_NONE && \
		FNPTR_GET( deviceInfoPtr->getRandomFunction ) == NULL )
		{
		DEBUG_PRINT(( "sanityCheckDevice: Random function" ));
		return( FALSE );
		}
	if( DATAPTR_GET( deviceInfoPtr->capabilityInfoList ) == NULL )
		{
		DEBUG_PRINT(( "sanityCheckDevice: Capability function" ));
		return( FALSE );
		}
		...
		case CRYPT_DEVICE_NONE:
			...
			if( systemInfo->nonceDataInitialised == FALSE )
				...
				if( !isEmptyData( systemInfo->nonceData, 
					...
					{
					DEBUG_PRINT(( "sanityCheckDevice: Spurious nonce RNG data" ));
					return( FALSE );
					}
				}
			else
				...
				if( systemInfo->nonceDataInitialised != TRUE || \
					...
					isEmptyData( systemInfo->nonceData, 
								 systemInfo->nonceHashSize ) )
					{
					DEBUG_PRINT(( "sanityCheckDevice: Nonce RNG data" ));
					return( FALSE );
					}
				}
		...
		case CRYPT_DEVICE_PKCS11:
			...
			if( pkcs11Info->minPinSize != 0 || pkcs11Info->maxPinSize != 0 )
				...
				if( pkcs11Info->minPinSize < 4 || \
					...
					{
					DEBUG_PRINT(( "sanityCheckDevice: PIN size" ));
					return( FALSE );
					}
				}
			...
			if( pkcs11Info->deviceNo < 0 || \
				...
				{
				DEBUG_PRINT(( "sanityCheckDevice: Device number" ));
				return( FALSE );
				}
			if( !isEmptyData( pkcs11Info->defaultSSOPin, \
				...
				if( pkcs11Info->defaultSSOPinLen <= 0 || \
					...
					{
					DEBUG_PRINT(( "sanityCheckDevice: SSO PIN size" ));
					return( FALSE );
					}
				}
...
static int openDevice( OUT_HANDLE_OPT CRYPT_DEVICE *iCryptDevice,
	...
	*deviceInfoPtrPtr = deviceInfoPtr;
	...
	if( cryptStatusError( status ) )
		return( status );
	ENSURES( FNPTR_GET( deviceInfoPtr->initFunction ) != NULL && \
			 FNPTR_GET( deviceInfoPtr->controlFunction ) != NULL );
	ENSURES( ( deviceInfoPtr->type == CRYPT_DEVICE_NONE ) || \
			 ( FNPTR_GET( deviceInfoPtr->getItemFunction ) != NULL && \
			   FNPTR_GET( deviceInfoPtr->setItemFunction ) != NULL && \
			   FNPTR_GET( deviceInfoPtr->deleteItemFunction ) != NULL && \
			   FNPTR_GET( deviceInfoPtr->getFirstItemFunction ) != NULL && \
	...
	ENSURES( ( deviceInfoPtr->type == CRYPT_DEVICE_NONE && \
			   FNPTR_GET( deviceInfoPtr->getRandomFunction ) != NULL ) || \
			 ( deviceInfoPtr->type != CRYPT_DEVICE_NONE ) );

	/* Connect to the device */
	initFunction = FNPTR_GET( deviceInfoPtr->initFunction );
