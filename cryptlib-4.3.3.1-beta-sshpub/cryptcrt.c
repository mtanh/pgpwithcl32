...
BOOLEAN sanityCheckCert(const CERT_INFO *certInfoPtr)
{
	...
	if (certInfoPtr->type <= CRYPT_CERTTYPE_NONE || \
		...
		certInfoPtr->flags < CERT_FLAG_NONE || \
		certInfoPtr->flags > CERT_FLAG_MAX || \
		certInfoPtr->version < 0 || \
		certInfoPtr->version > 3)
	{
		DEBUG_PRINT(("sanityCheckCert: General info"));
		return(FALSE);
	}
	if (certInfoPtr->certificate == NULL)
	{
		...
		{
			DEBUG_PRINT(("sanityCheckCert: Empty certificate data"));
			return(FALSE);
		}
	}
	else
	{
		...
		{
			DEBUG_PRINT(("sanityCheckCert: Certificate data"));
			return(FALSE);
		}
	}
	...
	if (certInfoPtr->subjectDNsize == 0)
	{
		...
		{
			DEBUG_PRINT(("sanityCheckCert: Empty subject DN"));
			return(FALSE);
		}
	}
	else
	{
		...
		/* Full PKIX allows null DNs (!!), so we have to handle the
		   ...
		   with this we check for the special case of there being a pointed
			...
			certInfoPtr->subjectDNptr == NULL)
		{
			...
			{
				DEBUG_PRINT(("sanityCheckCert: Null subject DN"));
				return(FALSE);
			}
			nullDN = TRUE;
		}
		if (certInfoPtr->subjectDNsize <= 8 || \
			certInfoPtr->subjectDNsize >= MAX_INTLENGTH_SHORT)
		{
			if (!nullDN && certInfoPtr->subjectName == NULL && \
				certInfoPtr->subjectDNptr != NULL && \
				!memcmp(certInfoPtr->subjectDNptr, "\x30\x00", 2))
				nullDN = TRUE;
			if (!nullDN)
			{
				DEBUG_PRINT(("sanityCheckCert: Non-NULL DN"));
				return(FALSE);
			}
		}
#else
		if (certInfoPtr->subjectName == NULL && \
			certInfoPtr->subjectDNptr == NULL)
		{
			DEBUG_PRINT(("sanityCheckCert: Absent subject DN"));
			return(FALSE);
		}
		if (certInfoPtr->subjectDNsize <= 8 || \
			certInfoPtr->subjectDNsize >= MAX_INTLENGTH_SHORT)
		{
			DEBUG_PRINT(("sanityCheckCert: Subject DN size"));
			return(FALSE);
		}
	...
	if (certInfoPtr->issuerDNsize == 0)
	{
		...
		{
			DEBUG_PRINT(("sanityCheckCert: Empty issuer DN"));
			return(FALSE);
		}
	}
	else
	{
		...
		{
			DEBUG_PRINT(("sanityCheckCert: Absent issuer DN"));
			return(FALSE);
		}
		if (certInfoPtr->issuerDNsize <= 8 || \
			certInfoPtr->issuerDNsize >= MAX_INTLENGTH_SHORT)
		{
			DEBUG_PRINT(("sanityCheckCert: Issuer DN size"));
			return(FALSE);
		}
	}
	...
	{
		DEBUG_PRINT(("sanityCheckCert: Object handles"));
		return(FALSE);
	}
	...
	case CRYPT_ICERTTYPE_SSL_CERTCHAIN:
	{
		...
		if (!((certInfo->serialNumber == NULL && \
			certInfo->serialNumberLength == 0) || \
			(certInfo->serialNumber != NULL && \
				certInfo->serialNumberLength > 0 && \
				certInfo->serialNumberLength <= 64)))
		{
			DEBUG_PRINT(("sanityCheckCert: Serial number"));
			return(FALSE);
		}
		if (certInfo->maxCheckLevel < CRYPT_COMPLIANCELEVEL_OBLIVIOUS || \
			certInfo->maxCheckLevel >= CRYPT_COMPLIANCELEVEL_LAST)
		{
			DEBUG_PRINT(("sanityCheckCert: Compliance level"));
			return(FALSE);
		}
		if (certInfo->chainEnd != 0 || certInfo->chainPos != CRYPT_ERROR)
		{
			/* It's a certificate chain, check that the chain
			   information is in order.  A chainPos of -1 is valid for
			   the leaf crtificate, which is past the end of the chain */
			if (certInfo->chainEnd < 0 || \
				certInfo->chainEnd >= MAX_CHAINLENGTH || \
				certInfo->chainPos < -1 || \
				certInfo->chainPos > certInfo->chainEnd)
			{
				DEBUG_PRINT(("sanityCheckCert: Chain info"));
				return(FALSE);
			}
		}
		...
		{
			DEBUG_PRINT(("sanityCheckCert: Hash algorithm"));
			return(FALSE);
		}
	...
	case CRYPT_CERTTYPE_REQUEST_REVOCATION:
	{
		...
		/* Check request data */
		if (!((reqInfo->serialNumber == NULL && \
			reqInfo->serialNumberLength == 0) || \
			(reqInfo->serialNumber != NULL && \
				reqInfo->serialNumberLength > 0 && \
				reqInfo->serialNumberLength <= 64)))
		{
			DEBUG_PRINT(("sanityCheckCert: Serial number"));
			return(FALSE);
		}
		...
		{
			DEBUG_PRINT(("sanityCheckCert: RA flag"));
			return(FALSE);
		}
	...
	case CRYPT_ICERTTYPE_REVINFO:
	{
		...
		{
			DEBUG_PRINT(("sanityCheckCert: Responder URL"));
			return(FALSE);
		}
		...
		{
			DEBUG_PRINT(("sanityCheckCert: Signature level"));
			return(FALSE);
		}
		if (revInfo->revocations != NULL && \
			!sanityCheckRevInfo(revInfo->revocations))
		{
			DEBUG_PRINT(("sanityCheckCert: Revocation info"));
			return(FALSE);
		}
		if (revInfo->currentRevocation != NULL && \
			!sanityCheckRevInfo(revInfo->currentRevocation))
		{
			DEBUG_PRINT(("sanityCheckCert: Current revocation"));
			return(FALSE);
		}
	...
	case CRYPT_CERTTYPE_RTCS_RESPONSE:
	{
		...
		{
			DEBUG_PRINT(("sanityCheckCert: Response type"));
			return(FALSE);
		}
		if (valInfo->validityInfo != NULL && \
			!sanityCheckValInfo(valInfo->validityInfo))
		{
			DEBUG_PRINT(("sanityCheckCert: Validity info"));
			return(FALSE);
		}
		if (valInfo->currentValidity != NULL && \
			!sanityCheckValInfo(valInfo->currentValidity))
		{
			DEBUG_PRINT(("sanityCheckCert: Current validity"));
			return(FALSE);
		}
	...
	case CRYPT_CERTTYPE_PKIUSER:
	{
		...
		/* Check PKI user data.  The only thing we can really check
		   is a single boolean */
		if (pkiUserInfo->isRA != TRUE && pkiUserInfo->isRA != FALSE)
		{
			DEBUG_PRINT(("sanityCheckCert: pkiUser RA flag"));
			return(FALSE);
		}
	...
	case CRYPT_CERTTYPE_CMS_ATTRIBUTES:
		...
		{
			DEBUG_PRINT(("sanityCheckCert: Spurious certificate info"));
			return(FALSE);
		}
		break;
