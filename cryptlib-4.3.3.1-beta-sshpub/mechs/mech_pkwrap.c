...
typedef struct {
	...
	const BYTE FAR_DATA *lHash;
	const int lHashSize;
	} LHASH_INFO;

static const LHASH_INFO FAR_DATA lHashInfo[] = {
...
static int mgf1( OUT_BUFFER_FIXED( maskLen ) void *mask, 
	...
	LOOP_MED( maskIndex = 0, maskIndex < maskLen, 
		...
		hashFunction( hashInfo, maskBuffer, hashSize, countBuffer, 4, 
					  HASH_STATE_END );
		REQUIRES( rangeCheckZ( maskIndex, noMaskBytes, maskLen ) );
...
static int generateOaepDataBlock( OUT_BUFFER_FIXED( dataMaxLen ) BYTE *data, 
	...
	REQUIRES( rangeCheck( 1 + seedLen, dbLen, dataMaxLen ) );
	memset( db, 0, dbLen );
	...
	REQUIRES( rangeCheck( 1 + seedLen + dbLen - messageLen, messageLen,
						  dataMaxLen ) );
...
static int recoverOaepDataBlock( OUT_BUFFER( messageMaxLen, *messageLen ) \
	...
	*messageLen = 0;
	...
	REQUIRES( rangeCheck( i, length, dbLen ) );
	REQUIRES( rangeCheckZ( 0, length, messageMaxLen ) );
