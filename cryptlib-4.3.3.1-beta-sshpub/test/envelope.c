...
BYTE FAR_DATA globalBuffer[BUFFER_SIZE];
...
static int testCMSDebugCheck(const TEST_TYPE testType, const FAULT_TYPE testFaultType)
{
	...
	if (status)
	{
		...
		puts("  (This test should have led to an enveloping failure but "
			"didn't, test has\n   failed).\n");
		return(FALSE);
	}
	...
	/* These tests are supposed to fail, so if this happens then the overall
	   test has succeeded */
	puts("  (This test checks error handling, so the failure response is "
		"correct).\n");
	return(TRUE);
}
