...
typedef struct {
	...
	const C_CHR FAR_DATA *url, FAR_DATA *user, FAR_DATA *password;
	const C_CHR FAR_DATA *caCertUrl;
...
static const SCEP_INFO FAR_DATA scepInfo[] = {
...
static const CERT_DATA FAR_DATA scepRequestData[] = {
...
static const CERT_DATA FAR_DATA scepPkiUserData[] = {
	/* Identification information */
	{ CRYPT_CERTINFO_COUNTRYNAME, IS_STRING, 0, TEXT( "NZ" ) },
	{ CRYPT_CERTINFO_ORGANIZATIONNAME, IS_STRING, 0, TEXT( "Dave's Wetaburgers" ) },
	{ CRYPT_CERTINFO_ORGANIZATIONALUNITNAME, IS_STRING, 0, TEXT( "Procurement" ) },
...
static int scepClientServer( const SCEP_TEST_TYPE testType )
	...
	status = connectSCEP( TRUE, testType );
...
int testSessionSCEPClientServer( void )
	{
	return( scepClientServer( SCEP_TEST_NORMAL ) );
	}
...
int testSessionSCEPCustomExtClientServer( void )
	...
	status = scepClientServer( SCEP_TEST_CUSTOMEXT );
