...
static const FAR_DATA URL_PARSE_INFO urlParseInfo[] = {
	/* IP address forms */
...
static const FAR_DATA URL_PARSE_INFO invalidUrlParseInfo[] = {
	/* Bad port */
...
static int connectSSH( const CRYPT_SESSION_TYPE sessionType,
	...
	if( isServer )
		...
		if( cryptStatusOK( status ) && testType != SSH_TEST_PUBKEYAUTH )
			{
...
dualThreadContinue:
	...
	if( cryptStatusError( status ) )
		...
		printExtError( cryptSession, isServer ? \
					   ...
					   "Attempt to activate SSH client session", status,
					   __LINE__ );
		if( isErrorTest || testType == SSH_TEST_PUBKEYAUTH_WRONGNAME )
			{
			/* These tests are supposed to fail, so if this happens then the 
			   overall test has succeeded */
			cryptDestroySession( cryptSession );
	...
	if( testType == SSH_TEST_PUBKEYAUTH_WRONGNAME )
		{
		printf( "Attempt to connect with invalid user name succeeded "
				"when it should\nhave failed, line %d.\n", __LINE__ );
		return( FALSE );
		}
