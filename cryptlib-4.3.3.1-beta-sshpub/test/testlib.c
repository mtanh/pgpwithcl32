...
{
	// This block of code only needs to be run the first time, to add the
	// client's public key to the database
#if 0
	CRYPT_CERTIFICATE cryptCert;

	checkCreateDatabaseKeysets();
	convertSSHtoCert("r:/test.ssh", "test", &cryptCert);
	addKeyToDatabase("testkeys", cryptCert);
#endif /* 0 */

	// This code tests the SSH server with public-key auth.  The first 
	// function runs a server that requires public-key auth, the second
	// runs a loopback test with client and server in separate threads.
	testSessionSSHServerPubkeyAuth();
	testSessionSSHClientServerPubkeyAuth();

#if 0
	int value;

	//	cryptSetAttribute( CRYPT_UNUSED, CRYPT_OPTION_CERT_COMPLIANCELEVEL, CRYPT_COMPLIANCELEVEL_PKIX_PARTIAL );
	//	cryptSetAttribute( CRYPT_UNUSED, CRYPT_OPTION_CERT_COMPLIANCELEVEL, CRYPT_COMPLIANCELEVEL_STANDARD );
	//	cryptSetAttribute( CRYPT_UNUSED, CRYPT_OPTION_CERT_COMPLIANCELEVEL, CRYPT_COMPLIANCELEVEL_REDUCED );
	//	cryptSetAttribute( CRYPT_UNUSED, CRYPT_OPTION_CERT_COMPLIANCELEVEL, CRYPT_COMPLIANCELEVEL_OBLIVIOUS );
	cryptGetAttribute(CRYPT_UNUSED, CRYPT_OPTION_CERT_COMPLIANCELEVEL, &value);
	xxxCertImport(cmd);
	//	xxxCertImport( "r:/WebServerSignedCer.der" );
	//	xxxCertImport( "r:/WebServerSignedCloneCer.der" );
	cleanupAndExit(EXIT_SUCCESS);
	//	testSessionSSHClientServer();
#endif
//	testKeysetDatabase();
	testSessionSCEPClientServer();
	//	testSessionSSHClientServerPubkeyAuth();
	//	testSessionSSHClientServerPubkeyAuthWrongName();
...
int main(int argc, char **argv)
{
	...
	puts("Copyright Peter Gutmann 1995 - 2017.");
#ifdef CONFIG_FUZZ
	puts("");
