...
static CAPABILITY_INFO FAR_DATA capabilityTemplates[] = {
...
static CAPABILITY_INFO *getCapability( const DEVICE_INFO *deviceInfo,
	...
	PKCS11_INFO *pkcs11Info = deviceInfo->devicePKCS11;
	int hardwareOnly, noMechanisms, i, LOOP_ITERATOR;
...
static int getCapabilities( DEVICE_INFO *deviceInfo,
	...
	if( capabilityInfoListTail != NULL )
		{
		LOOP_MED_CHECK( DATAPTR_GET( capabilityInfoListTail->next ) != NULL )
			{
	...
	LOOP_MED( i = 0, i < maxMechanisms && \
		...
		if( DATAPTR_GET( deviceInfo->capabilityInfoList ) == NULL )
			{
	...
	return( ( DATAPTR_GET( deviceInfo->capabilityInfoList ) == NULL ) ? \
...
static int initFunction( DEVICE_INFO *deviceInfo, const char *name,
	...
	labelLength = strStripWhitespace( &labelPtr, tokenInfo.label, 32 );
	if( labelLength > 0 )
		{
