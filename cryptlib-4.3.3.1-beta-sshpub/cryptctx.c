...
BOOLEAN sanityCheckContext( const CONTEXT_INFO *contextInfoPtr )
	...
	if( contextInfoPtr->type <= CONTEXT_NONE || \
		...
		contextInfoPtr->flags < CONTEXT_FLAG_NONE || \
		contextInfoPtr->flags > CONTEXT_FLAG_MAX )
		{
		DEBUG_PRINT(( "sanityCheckContext: General info" ));
		return( FALSE );
		}
	...
	if( contextInfoPtr->flags & CONTEXT_FLAG_STATICCONTEXT )
		...
		if( contextInfoPtr->objectHandle != 0 || \
			...
			{
			DEBUG_PRINT(( "sanityCheckContext: Spurious object handles" ));
			return( FALSE );
			}
		}
	else
		...
		if( !isHandleRangeValid( contextInfoPtr->objectHandle ) || \
			...
			{
			DEBUG_PRINT(( "sanityCheckContext: Object handles" ));
			return( FALSE );
			}
		}
	...
	switch( contextInfoPtr->type )
		...
		case CONTEXT_CONV:
			...
			if( convInfo->mode < CRYPT_MODE_NONE || \
				...
				{
				DEBUG_PRINT(( "sanityCheckContext: Mode" ));
				return( FALSE );
				}

			/* Check key and IV data */
			if( !( isEmptyData( convInfo->userKey, 
								convInfo->userKeyLength ) || \
				   ( convInfo->userKeyLength > 0 && \
					 convInfo->userKeyLength <= CRYPT_MAX_KEYSIZE ) ) )
				{
				DEBUG_PRINT(( "sanityCheckContext: User key" ));
				return( FALSE );
				}
			if( !( isEmptyData( convInfo->iv, 
								convInfo->ivLength ) || \
				   ( convInfo->ivLength > 0 && \
					 convInfo->ivLength <= CRYPT_MAX_IVSIZE ) ) )
				{
				DEBUG_PRINT(( "sanityCheckContext: IV" ));
				return( FALSE );
				}

			/* Check PRF data */
			if( !( isEmptyData( convInfo->salt, 
								convInfo->saltLength ) || \
				   ( convInfo->saltLength > 0 && \
					 convInfo->saltLength <= CRYPT_MAX_HASHSIZE ) ) )
				{
				DEBUG_PRINT(( "sanityCheckContext: Salt" ));
				return( FALSE );
				}
			if( convInfo->keySetupIterations < 0 || \
				convInfo->keySetupIterations > MAX_KEYSETUP_ITERATIONS )
				{
				DEBUG_PRINT(( "sanityCheckContext: Key setup iterations" ));
				return( FALSE );
				}
			if( convInfo->keySetupAlgorithm != CRYPT_ALGO_NONE && \
				...
				{
				DEBUG_PRINT(( "sanityCheckContext: Key setup algorithm" ));
				return( FALSE );
				}
		...
		case CONTEXT_PKC:
			...
			/* The PKC info is sufficiently complex that it has its own
			   checking function */
			if( !sanityCheckPKCInfo( contextInfoPtr->ctxPKC ) )
				{
				DEBUG_PRINT(( "sanityCheckContext: PKC info" ));
				return( FALSE );
				}
		...
		case CONTEXT_HASH:
			/* Hash contexts have no specific data to check */
			break;

		case CONTEXT_MAC:
			{
			const MAC_INFO *macInfo = contextInfoPtr->ctxMAC;

			assert( isReadPtr( macInfo, sizeof( MAC_INFO ) ) );

			/* Check key data */
			if( !( isEmptyData( macInfo->userKey, 
								macInfo->userKeyLength ) || \
				   ( macInfo->userKeyLength > 0 && \
					 macInfo->userKeyLength <= CRYPT_MAX_KEYSIZE ) ) )
				{
				DEBUG_PRINT(( "sanityCheckContext: User key" ));
				return( FALSE );
				}
			...
			/* Check PRF data */
			if( !( isEmptyData( macInfo->salt, 
								macInfo->saltLength ) || \
				   ( macInfo->saltLength > 0 && \
					 macInfo->saltLength <= CRYPT_MAX_HASHSIZE ) ) )
				{
				DEBUG_PRINT(( "sanityCheckContext: Salt" ));
				return( FALSE );
				}
		...
		case CONTEXT_GENERIC:
			{
			const GENERIC_INFO *genericInfo = contextInfoPtr->ctxGeneric;

			assert( isReadPtr( genericInfo, sizeof( GENERIC_INFO ) ) );

			/* Check key data */
			if( !( isEmptyData( genericInfo->genericSecret, 
								genericInfo->genericSecretLength ) || \
				   ( genericInfo->genericSecretLength > 0 && \
					 genericInfo->genericSecretLength <= CRYPT_MAX_KEYSIZE ) ) )
				{
				DEBUG_PRINT(( "sanityCheckContext: Generic secret" ));
				return( FALSE );
				}

			/* Check KDF data */
			if( !( isEmptyData( genericInfo->kdfParams, 
								genericInfo->kdfParamSize ) || \
				   ( genericInfo->kdfParamSize > 0 && \
					 genericInfo->kdfParamSize <= CRYPT_MAX_TEXTSIZE ) ) )
				{
				DEBUG_PRINT(( "sanityCheckContext: KDF parameters" ));
				return( FALSE );
				}
			if( !( isEmptyData( genericInfo->encAlgoParams, 
								genericInfo->encAlgoParamSize ) || \
				   ( genericInfo->encAlgoParamSize > 0 && \
					 genericInfo->encAlgoParamSize <= CRYPT_MAX_TEXTSIZE ) ) )
				{
				DEBUG_PRINT(( "sanityCheckContext: Enc algorithm parameters" ));
				return( FALSE );
				}
			if( !( isEmptyData( genericInfo->macAlgoParams, 
								genericInfo->macAlgoParamSize ) || \
				   ( genericInfo->macAlgoParamSize > 0 && \
					 genericInfo->macAlgoParamSize <= CRYPT_MAX_TEXTSIZE ) ) )
				{
				DEBUG_PRINT(( "sanityCheckContext: MAC algorithm parameters" ));
				return( FALSE );
				}
...
static int initContextStorage( INOUT CONTEXT_INFO *contextInfoPtr, 
	...
	/* We can't call sanityCheckContext() at this point since the storage
	...
	if( message == MESSAGE_COMPARE )
		...
		switch( messageValue )
			...
			case MESSAGE_COMPARE_KEYID_PGP:
				...
				if( ( contextInfoPtr->ctxPKC->flags & PKCINFO_FLAG_PGPKEYID_SET ) && \
			...
			case MESSAGE_COMPARE_KEYID_OPENPGP:
				...
				if( ( contextInfoPtr->ctxPKC->flags & PKCINFO_FLAG_OPENPGPKEYID_SET ) && \
					msgData->length == PGP_KEYID_SIZE && \
...
int createContextFromCapability( OUT_HANDLE_OPT CRYPT_CONTEXT *iCryptContext,
	...
	*iCryptContext = CRYPT_ERROR;
	...
	if( objectFlags & CREATEOBJECT_FLAG_DUMMY )
		...
		contextInfoPtr->flags |= CONTEXT_FLAG_DUMMY;
		if( contextInfoPtr->type == CONTEXT_PKC )
			contextInfoPtr->ctxPKC->flags |= PKCINFO_FLAG_DUMMY;
		}
...
int createContext( INOUT MESSAGE_CREATEOBJECT_INFO *createInfo,
	...
	const CAPABILITY_INFO FAR_DATA *capabilityInfoPtr;
	int status;
