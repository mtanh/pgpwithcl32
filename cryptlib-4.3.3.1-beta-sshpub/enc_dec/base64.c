...
static const char FAR_DATA binToAscii[] = \
	"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";

static const BYTE FAR_DATA asciiToBin[ 256 ] = { 
...
/* The size of the buffe for reading a line of text */
...
typedef struct {
	...
	const char FAR_DATA *header;
	const int headerLen;
	BUFFER_FIXED( trailerLen ) \
	const char FAR_DATA *trailer;
	const int trailerLen;
	} HEADER_INFO;
static const HEADER_INFO FAR_DATA headerInfo[] = {
...
static int checkPEMHeader( INOUT STREAM *stream, 
	...
	*headerLength = 0;
	...
	LOOP_LARGE_CHECKINC( length >= 4, length-- )
		{
		if( *bufPtr == '-' )
			break;
		bufPtr++;
...
int base64checkHeader( IN_BUFFER( dataLength ) const BYTE *data, 
	...
	*startPos = 0;
	...
	/* If the item is too small to contain any useful data then we don't 
	   even try and examine it.  We don't treat this as a data or underflow 
	   error since it may be a short but valid data object like an empty 
	   CRL */
	if( dataLength < 64 )
		return( CRYPT_OK );
	...
	LOOP_MED( ( length = 0, lineCount = 0 ), 
			  length <= 0 && lineCount < 30, lineCount++ )
		{
	...
	ENSURES( LOOP_BOUND_OK );
	if( lineCount >= 30 || !seenTransferEncoding )
		{
		sMemDisconnect( &stream );
		return( CRYPT_ERROR_BADDATA );
		}
...
/* Decode a base64-encoded binary object.  This has to resort to a somewhat
   ...
   so that part of a base64-encoded block can start one one line but 
   continue on the next.  To deal with this, we shift the decoded data into
   an accumulator and write out its contents once it's absorbed a full 
   block.  This means that we can skip intervening EOL characters without
   needing to have a full block on each line */
...
int base64decode( OUT_BUFFER( destMaxLen, *destLen ) void *dest, 
	...
	*destLen = 0;
	...
	/* If there's a single byte left, this is an error since it can't be 
...
int base64decodeLen( IN_BUFFER( dataLength ) const BYTE *data, 
	...
	{
	STREAM stream;
	int ch, length DUMMY_INIT, i, LOOP_ITERATOR;
	...
	*decodedLength = 0;
	...
	sMemConnect( &stream, data, dataLength );
	LOOP_EXT( i = 0, i < MAX_BUFFER_SIZE, i++, MAX_BUFFER_SIZE )
		{
		length = stell( &stream );
		ch = sgetc( &stream );
