...
static int readLengthValue( INOUT STREAM *stream, 
	...
	BOOLEAN shortLen = ( readType == READLENGTH_SHORT || \
					   ...
					   TRUE : FALSE;
	long dataLength;
	int noLengthOctets, i, status, LOOP_ITERATOR;
	...
	*length = 0;
	...
	if( buffer[ 0 ] == 0 )
		...
		LOOP_SMALL( i = 0, i < noLengthOctets && buffer[ i ] == 0, i++ );
	...
	LOOP_SMALL( ( dataLength = 0, i = 0 ), i < noLengthOctets, i++ )
		{
...
static int readNumeric( INOUT STREAM *stream, OUT_OPT_LENGTH_Z long *value )
	...
	/* Read the data and convert it to an integer value.  Note that although 
	   the variable that the value is being read into is a long, we restrict 
	   the value itself to an integer range because most of the code that 
	   works with it expects an integer and will reject a long even if we 
	   allow it at this point.  This only affects 16-bit systems in any 
	   case */
	status = sread( stream, buffer, length );
	if( cryptStatusError( status ) || value == NULL )
		return( status );
	LOOP_SMALL( ( localValue = 0, i = 0 ), i < length, i++ )
		{
...
int readBitStringTag( INOUT STREAM *stream, 
					  ...
					  IN_TAG_EXT const int tag )
	{
	int length, data, mask, flag, value = 0, noBits, i, LOOP_ITERATOR;
	...
	ENSURES_S( LOOP_BOUND_OK );
	LOOP_MED( ( flag = 1, i = 0 ), i < noBits, ( flag <<= 1, i++ ) )
		{
		if( data & mask )
			value |= flag;
		data <<= 1;
