...
// C# API
//static void convertSSHtoCert(const char *fileName,
//	const char *userName,
//	CRYPT_CERTIFICATE *cryptCertPtr)
//{
//	C_RET cryptSetSshKey(C_IN CRYPT_CONTEXT publicKey,
//		C_IN void *publicKeyData,
//		C_IN int publicKeyDataLength);
//	CRYPT_CONTEXT cryptContext, cryptCAKey;
//	CRYPT_CERTIFICATE cryptCert;
//	FILE *filePtr;
//	BYTE buffer[BUFFER_SIZE];
//	int fileNo = 1, count, status;
//
//	/* Read the SSH public key into a context */
//	filePtr = fopen(fileName, "rb");
//	if (filePtr == NULL)
//	{
//		printf("Couldn't open public-key file '%s', line %d.\n",
//			fileName, __LINE__);
...
//	if (cryptStatusOK(status))
//		status = cryptSetSshKey(cryptContext, buffer, count);
//	if (cryptStatusError(status))
//	{
//		printf("Couldn't import SSH public key, status %d, line %d.\n",
//			status, __LINE__);
//		cleanupAndExit(EXIT_FAILURE);
//	}
//
//	/* Convert the context into a certificate for the given user */
//	status = getPrivateKey(&cryptCAKey, /*CA_PRIVKEY_FILE*/TEXT("D:/ca.p15"),
//		/*CA_PRIVKEY_LABEL*/TEXT("Test RSA private key"), /*TEST_PRIVKEY_PASSWORD*/TEXT("test"));
//	if (cryptStatusError(status))
//	{
...
//	if (cryptStatusError(status))
//	{
//		printf("Couldn't create certificate, status %d, line %d.\n",
//			status, __LINE__);
//		cleanupAndExit(EXIT_FAILURE);
//	}
...
//	if (cryptStatusError(status))
//	{
//		printf("Couldn't sign certificate, status %d, line %d.\n",
//			status, __LINE__);
//		cleanupAndExit(EXIT_FAILURE);
//	}
//	cryptDestroyContext(cryptCAKey);
//
//	*cryptCertPtr = cryptCert;
//}
...
//static void addKeyToDatabase(const char *database,
//	const CRYPT_CERTIFICATE cryptCert)
C_RET addKeyToDatabase(char C_PTR dsn,
	CRYPT_CERTIFICATE cryptCert)
{
	CRYPT_KEYSET cryptKeyset;
	int status;

	status = cryptKeysetOpen(&cryptKeyset, CRYPT_UNUSED, CRYPT_KEYSET_ODBC,
		dsn, CRYPT_KEYOPT_CREATE);
	if (cryptStatusOK(status))
	{
		DEBUG_PRINT(("Created new certificate database '%s'.\n", dsn));
	...
	else
	{
		...
		if (status == CRYPT_ERROR_DUPLICATE)
		{
			DEBUG_PRINT(("addKeyToDatabase %d.\n", __LINE__));
			status = cryptKeysetOpen(&cryptKeyset, CRYPT_UNUSED,
				CRYPT_KEYSET_ODBC, dsn,
				CRYPT_KEYOPT_NONE);
		}
	}
	if (cryptStatusError(status))
	{
		...
		printf("cryptKeysetOpen() of keyset '%s' failed with error code "
			"%d, line %d.\n", dsn, status, __LINE__);
		DEBUG_PRINT(("cryptKeysetOpen() of keyset '%s' failed with error code "
			"%d, line %d.\n", dsn, status, __LINE__));
		cleanupAndExit(EXIT_FAILURE);
	}
...
C_CHECK_RETVAL \
C_RET dicConvertAndImportSSHCert(C_STR dsn,
	C_STR publicKeyFilePath,
	C_STR keyAlias)
{
	CRYPT_CERTIFICATE cryptCert;

	//checkCreateDatabaseKeysets();
	convertSSHtoCert(publicKeyFilePath, keyAlias, &cryptCert, NULL);
	addKeyToDatabase(dsn, cryptCert);
...
{
	static const COMMAND_INFO FAR_DATA cmdTemplate = \
	{ COMMAND_CREATEOBJECT, COMMAND_FLAG_NONE, 3, 0,
	{ SYSTEM_OBJECT_HANDLE, OBJECT_TYPE_CONTEXT } };
	static const ERRORMAP FAR_DATA errorMap[] = \
	...
	IN_FLAGS_Z(TRANSPORT) const int flags)
{
	...
	*length = 0;
	...
	/* This function is never called since all reads are satisfied by the
	   read buffering layer.  In other words the stream-level buffer contains
	   all the data that's needed, so the transport read function is never
	   called */
	assert(DEBUG_WARN);
...
C_RET cryptSetSshKey(C_IN CRYPT_CONTEXT publicKey,
	...
	C_IN int publicKeyDataLength)
{
	CRYPT_CERTFORMAT_TYPE type;
	STREAM stream;
	MESSAGE_DATA msgData;
	BYTE sshKey[4096];
	int sshKeySize, startPos, status;

	//DEBUG_PRINT(("cryptSetSshKey %d.\n", __LINE__));

	if (publicKeyDataLength < 64 || publicKeyDataLength > 4096)
	...
	status = base64decode(sshKey + 4, 4096 - 4, &sshKeySize,
		...
		publicKeyDataLength - startPos, type);
	if (cryptStatusError(status))
	{
		//DEBUG_PRINT(("cryptSetSshKey %d.\n", __LINE__));
		return(status);
	...
	/* Add the length field at the start */
	sMemOpen(&stream, sshKey, 4);
	//DEBUG_PRINT(("cryptSetSshKey %d.\n", __LINE__));
	writeUint32(&stream, sshKeySize);
	//DEBUG_PRINT(("cryptSetSshKey %d.\n", __LINE__));
	sMemDisconnect(&stream);
	//DEBUG_PRINT(("cryptSetSshKey %d.\n", __LINE__));

	setMessageData(&msgData, sshKey, sshKeySize + 4);
	//DEBUG_PRINT(("cryptSetSshKey %d.\n", __LINE__));
	status = krnlSendMessage(publicKey, IMESSAGE_SETATTRIBUTE_S, &msgData,
		CRYPT_IATTRIBUTE_KEY_SSH/*CRYPT_IATTRIBUTE_KEY_PGP*/);
	//DEBUG_PRINT(("cryptSetSshKey status: %d.\n", status));

	return (status);
}

C_RET cryptSetPgpKey(C_IN CRYPT_CONTEXT publicKey,
	C_IN void *publicKeyData,
	C_IN int publicKeyDataLength)
{
	CRYPT_CERTFORMAT_TYPE type;
...
C_RET convertSSHtoCert(char C_PTR fileName,
	char C_PTR keyAlias,
	CRYPT_CERTIFICATE C_PTR cryptCertPtr,
	void* userData)
{
	/*C_RET cryptSetSshKey(C_IN CRYPT_CONTEXT publicKey,
	C_IN void *publicKeyData,
	...
	const time_t theNextTenYearsTime = theTime + (3650 * 24 * 60 * 60);
	...
	if (cryptStatusError(status))
	{
		...
		DEBUG_PRINT(("Couldn't reload cryptlib configuration.\n"));
		return;
	}*/

	/*DEBUG_PRINT(("convertSSHtoCert %d.\n",
		__LINE__));*/
	...
	status = cryptCreateContext(&cryptContext, CRYPT_UNUSED, CRYPT_ALGO_RSA);
	//DEBUG_PRINT(("convertSSHtoCert -- cryptContext: %d, status: %d\n", cryptContext, status));
	...
	if (cryptStatusError(status))
	{
		/*printf("Couldn't import SSH public key, status %d, line %d.\n",
			status, __LINE__);*/
			/*DEBUG_PRINT(("Couldn't import SSH public key, status %d, line %d.\n",
				status, __LINE__));*/
		cleanupAndExit(EXIT_FAILURE);
	}
	...
	/*DEBUG_PRINT(("convertSSHtoCert %d.\n",
		...
		/* Convert the context into a certificate for the given user */
	dicUserDataBundle* userDataPtr = (dicUserDataBundle*)userData;
	char* cfp = "C:/DiCentral/DiConnect Protocol Utilities/ca.p15";
	if (userDataPtr != NULL)
	{
		cfp = userDataPtr->m_caFilePath;
	}
	status = getPrivateKey(&cryptCAKey, /*CA_PRIVKEY_FILE*/cfp,
		/*CA_PRIVKEY_LABEL*/TEXT("Test RSA private key"), /*TEST_PRIVKEY_PASSWORD*/TEXT("test"));
	if (cryptStatusError(status))
	{
		printf("CA private key read failed with error code %d, line %d.\n",
...
C_RET convertPGPtoCert(char C_PTR fileName,
	char C_PTR keyAlias,
	CRYPT_CERTIFICATE C_PTR cryptCertPtr,
	void* userData)
{
	...
	if (cryptStatusOK(status))
	{
		status = cryptSetPgpKey(cryptContext, buffer, count);
	}
