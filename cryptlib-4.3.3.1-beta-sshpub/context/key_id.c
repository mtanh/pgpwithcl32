...
static int calculateKeyIDFromEncoded( INOUT CONTEXT_INFO *contextInfoPtr,
	...
	if( isPgpAlgo )
		{
		if( staticContextData.flags & PKCINFO_FLAG_PGPKEYID_SET )
			{
			memcpy( publicKey->pgp2KeyID, staticContextData.pgp2KeyID, 
					PGP_KEYID_SIZE );
			contextInfoPtr->ctxPKC->flags |= PKCINFO_FLAG_PGPKEYID_SET;
			}
		if( staticContextData.flags & PKCINFO_FLAG_OPENPGPKEYID_SET )
			{
			memcpy( publicKey->openPgpKeyID, staticContextData.openPgpKeyID, 
					PGP_KEYID_SIZE );
			contextInfoPtr->ctxPKC->flags |= PKCINFO_FLAG_OPENPGPKEYID_SET;
			}
		}
	staticDestroyContext( &staticContextInfo );
