...
static BOOLEAN sanityCheckFunctionality( const CAPABILITY_INFO *capabilityInfoPtr,
	...
	if( isSpecialAlgo( cryptAlgo ) )
		...
		if( capabilityInfoPtr->encryptFunction != NULL || \
			...
			isCrypt || isSig )
			{
			DEBUG_PRINT(( "sanityCheckFunctionality: Generic secret" ));
			return( FALSE );
			}
	...
	if( ( capabilityInfoPtr->encryptFunction == NULL || \
		...
		{
		DEBUG_PRINT(( "sanityCheckFunctionality: General capabilities" ));
		return( FALSE );
		}
	...
	if( isConvAlgo( cryptAlgo ) )
		...
		if( isSig )
			{
			DEBUG_PRINT(( "sanityCheckFunctionality: Spurious conv capability" ));
			return( FALSE );
			}
		if( isStreamCipher( cryptAlgo ) )
			...
			if( capabilityInfoPtr->encryptCFBFunction == NULL || \
				capabilityInfoPtr->decryptCFBFunction == NULL )
				{
				DEBUG_PRINT(( "sanityCheckFunctionality: Missing CFB" ));
				return( FALSE );
				}
			if( capabilityInfoPtr->encryptFunction != NULL || \
				capabilityInfoPtr->decryptFunction != NULL || \
				...
				capabilityInfoPtr->decryptGCMFunction != NULL )
				{
				DEBUG_PRINT(( "sanityCheckFunctionality: Spurious stream capability" ));
				return( FALSE );
				}
			}
		else
			...
			if( capabilityInfoPtr->encryptFunction == NULL && \
				...
				!isCrypt )
				{
				DEBUG_PRINT(( "sanityCheckFunctionality: Missing crypt capability" ));
				return( FALSE );
				}
			}
		if( ( capabilityInfoPtr->encryptCBCFunction != NULL && \
			...
			( capabilityInfoPtr->encryptCBCFunction == NULL && \
			  capabilityInfoPtr->decryptCBCFunction != NULL ) )
			{
			DEBUG_PRINT(( "sanityCheckFunctionality: Inconsistent CBC" ));
			return( FALSE );
			}
		if( ( capabilityInfoPtr->encryptCFBFunction != NULL && \
			  capabilityInfoPtr->decryptCFBFunction == NULL ) || \
			( capabilityInfoPtr->encryptCFBFunction == NULL && \
			  capabilityInfoPtr->decryptCFBFunction != NULL ) )
			{
			DEBUG_PRINT(( "sanityCheckFunctionality: Inconsistent CFB" ));
			return( FALSE );
			}
		if( ( capabilityInfoPtr->encryptGCMFunction != NULL && \
			...
			( capabilityInfoPtr->encryptGCMFunction == NULL && \
			  capabilityInfoPtr->decryptGCMFunction != NULL ) )
			{
			DEBUG_PRINT(( "sanityCheckFunctionality: Inconsistent GCM" ));
			return( FALSE );
			}
	...
	if( isPkcAlgo( cryptAlgo ) )
		...
		if( capabilityInfoPtr->encryptFunction == NULL && \
			...
			capabilityInfoPtr->sigCheckFunction == NULL )
			{
			DEBUG_PRINT(( "sanityCheckFunctionality: Missing PKC capability" ));
			return( FALSE );
			}
		if( isCrypt )
			{
			DEBUG_PRINT(( "sanityCheckFunctionality: Spurious PKC capability" ));
			return( FALSE );
			}
	...
	if( isHashAlgo( cryptAlgo ) || isMacAlgo( cryptAlgo ) )
		...
		if( capabilityInfoPtr->encryptFunction == NULL || \
			capabilityInfoPtr->decryptFunction == NULL )
			{
			DEBUG_PRINT(( "sanityCheckFunctionality: Missing hash/MAC capability" ));
			return( FALSE );
			}
		if( isCrypt || isSig )
			{
			DEBUG_PRINT(( "sanityCheckFunctionality: Spurious hash/MAC capability" ));
			return( FALSE );
			}
...
BOOLEAN sanityCheckCapability( const CAPABILITY_INFO *capabilityInfoPtr )
	...
	if( cryptAlgo <= CRYPT_ALGO_NONE || cryptAlgo >= CRYPT_ALGO_LAST )
		{
		DEBUG_PRINT(( "sanityCheckCapability: Algorithm" ));
		return( FALSE );
		}
	if( capabilityInfoPtr->algoName == NULL || \
		...
		{
		DEBUG_PRINT(( "sanityCheckCapability: Algorithm name" ));
		return( FALSE );
		}
	...
	if( capabilityInfoPtr->selfTestFunction != NULL )
		{
		DEBUG_PRINT(( "sanityCheckCapability: Suprious self-test function" ));
		return( FALSE );
		}
#else
	if( capabilityInfoPtr->selfTestFunction == NULL )
		{
		DEBUG_PRINT(( "sanityCheckCapability: Self-test function" ));
		return( FALSE );
		}
#endif /* CONFIG_NO_SELFTEST */
	if( capabilityInfoPtr->getInfoFunction == NULL )
		{
		DEBUG_PRINT(( "sanityCheckCapability: Get-info function" ));
		return( FALSE );
		}
	if( !sanityCheckFunctionality( capabilityInfoPtr, cryptAlgo ) )
		{
		DEBUG_PRINT(( "sanityCheckCapability: Functionality" ));
		return( FALSE );
		}
	...
	if( capabilityInfoPtr->minKeySize > capabilityInfoPtr->keySize || \
		...
		{
		DEBUG_PRINT(( "sanityCheckCapability: Key size" ));
		return( FALSE );
		}
	if( isConvAlgo( cryptAlgo ) )
		{
		if( ( capabilityInfoPtr->blockSize < bitsToBytes( 8 ) || \
			...
			{
			DEBUG_PRINT(( "sanityCheckCapability: Conv. block size" ));
			return( FALSE );
			}
		if( capabilityInfoPtr->keySize > MAX_WORKING_KEYSIZE )
			{
			DEBUG_PRINT(( "sanityCheckCapability: Conv. key wrap size" ));
			return( FALSE );	/* Requirement for key wrap */
			}
		if( capabilityInfoPtr->initParamsFunction == NULL || \
			capabilityInfoPtr->initKeyFunction == NULL )
			{
			DEBUG_PRINT(( "sanityCheckCapability: Conv. init functions" ));
			return( FALSE );
			}
		if( !isStreamCipher( cryptAlgo ) && \
			 capabilityInfoPtr->blockSize < bitsToBytes( 64 ) )
			{
			DEBUG_PRINT(( "sanityCheckCapability: Conv. minimum block size" ));
			return( FALSE );
			}
	...
	if( isPkcAlgo( cryptAlgo ) )
		...
		if( capabilityInfoPtr->blockSize != 0 || \
			...
			{
			DEBUG_PRINT(( "sanityCheckCapability: PKC key size" ));
			return( FALSE );
			}
		if( capabilityInfoPtr->initKeyFunction == NULL || \
			capabilityInfoPtr->generateKeyFunction == NULL )
			{
			DEBUG_PRINT(( "sanityCheckCapability: PKC Init/generate function" ));
			return( FALSE );
			}
	...
	if( isHashAlgo( cryptAlgo ) )
		...
		if( ( capabilityInfoPtr->blockSize < bitsToBytes( 128 ) || \
			...
			{
			DEBUG_PRINT(( "sanityCheckCapability: Hash block size" ));
			return( FALSE );
			}
	...
	if( isMacAlgo( cryptAlgo ) )
		...
		if( ( capabilityInfoPtr->blockSize < bitsToBytes( 128 ) || \
			...
			( capabilityInfoPtr->minKeySize < MIN_KEYSIZE || \
			  capabilityInfoPtr->maxKeySize > CRYPT_MAX_KEYSIZE ) )
			{
			DEBUG_PRINT(( "sanityCheckCapability: MAC key size" ));
			return( FALSE );
			}
		if( capabilityInfoPtr->keySize > MAX_WORKING_KEYSIZE )
			{
			DEBUG_PRINT(( "sanityCheckCapability: MAC key wrap size" ));
			return( FALSE );	/* Requirement for key wrap */
			}
		if( capabilityInfoPtr->initKeyFunction == NULL )
			{
			DEBUG_PRINT(( "sanityCheckCapability: MAC init function" ));
			return( FALSE );
			}
	...
	if( isSpecialAlgo( cryptAlgo ) )
		...
		if( capabilityInfoPtr->blockSize != 0 || \
			...
			capabilityInfoPtr->maxKeySize > CRYPT_MAX_KEYSIZE )
			{
			DEBUG_PRINT(( "sanityCheckCapability: Generic key size" ));
			return( FALSE );
			}
		if( capabilityInfoPtr->initKeyFunction == NULL )
			{
			DEBUG_PRINT(( "sanityCheckCapability: Generic init function" ));
			return( FALSE );
			}
...
const CAPABILITY_INFO FAR_DATA *findCapabilityInfo(
...
void getHashParameters( IN_ALGO const CRYPT_ALGO_TYPE hashAlgorithm,
	...
	{
	static const HASHFUNCTION_INFO FAR_DATA hashFunctions[] = {
#ifdef USE_MD5
...
void getHashAtomicParameters( IN_ALGO const CRYPT_ALGO_TYPE hashAlgorithm,
							  ...
							  OUT_OPT_LENGTH_SHORT_Z int *hashOutputSize )
	{
	static const HASHFUNCTION_ATOMIC_INFO FAR_DATA hashFunctions[] = {
#ifdef USE_MD5
