...
BOOLEAN sanityCheckRevInfo( const REVOCATION_INFO *revocationInfo )
	{
	assert( isReadPtr( revocationInfo, sizeof( REVOCATION_INFO ) ) );

	if( revocationInfo == NULL )
		{
		DEBUG_PRINT(( "sanityCheckRevInfo: Missing revocation info" ));
		return( FALSE );
		}

	/* Make sure that the ID type is valid */
	if( revocationInfo->idType != CRYPT_KEYID_NONE && \
		...
		{
		DEBUG_PRINT(( "sanityCheckRevInfo: ID type" ));
		return( FALSE );
		}

	/* Make sure that the ID data is valid */
	if( revocationInfo->id != revocationInfo->value || \
		revocationInfo->id == NULL || \
		revocationInfo->idLength <= 0 || \
		revocationInfo->idLength >= MAX_INTLENGTH_SHORT ) 
		{
		DEBUG_PRINT(( "sanityCheckRevInfo: ID" ));
		return( FALSE );
		}
	if( checksumData( revocationInfo->id, 
					  revocationInfo->idLength ) != revocationInfo->idCheck )
		{
		DEBUG_PRINT(( "sanityCheckRevInfo: Revocation ID" ));
		return( FALSE );
		}
...
static int findRevocationEntry( const REVOCATION_INFO *listPtr,
	...
	*insertPoint = NULL;
	...
	LOOP_MAX_CHECKINC( listPtr != NULL, listPtr = listPtr->next )
		{
		REQUIRES( sanityCheckRevInfo( listPtr ) );
...
void deleteRevocationEntries( INOUT_PTR REVOCATION_INFO **listHeadPtrPtr )
	...
	*listHeadPtrPtr = NULL;
	...
	LOOP_MAX_CHECK( entryListPtr != NULL )
		{
...
int prepareRevocationEntries( INOUT_OPT REVOCATION_INFO *listPtr, 
	...
	REQUIRES( listPtr == NULL || sanityCheckRevInfo( listPtr ) );
	...
	*errorEntry = NULL;
	...
	LOOP_LARGE( revocationEntry = listPtr, revocationEntry != NULL,
...
static int checkRevocationCRL( const CERT_INFO *certInfoPtr, 
	...
	REQUIRES( sanityCheckCert( revocationInfoPtr ) );
...
int copyRevocationEntries( INOUT_PTR REVOCATION_INFO **destListHeadPtrPtr,
	...
	REQUIRES( sanityCheckRevInfo( srcListPtr ) );
...
int checkOCSPResponse( INOUT CERT_INFO *certInfoPtr,
	...
	LOOP_LARGE( revocationInfo = certInfoPtr->cCertRev->revocations, 
				revocationInfo != NULL, 
...
static int sizeofOcspID( const REVOCATION_INFO *ocspEntry )
	...
	REQUIRES( sanityCheckRevInfo( ocspEntry ) );
