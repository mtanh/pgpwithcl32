	...
	IN_TAG const int tag)
{
	...
	LOOP_MED(dnComponentPtr = (DN_COMPONENT *)dnComponentList,
		dnComponentPtr != NULL,
		dnComponentPtr = dnComponentPtr->next)
	{
		const DN_COMPONENT_INFO *dnComponentInfo = dnComponentPtr->typeInfo;
	...
	IN_LENGTH_ATTRIBUTE const int stringLength)
{
	...
	*dnStringInfoIndex = 0;
		...
		stringInfoIndex++, MAX_DNSTRING_COMPONENTS + 1)
	{
		...
		ENSURES_B(rangeCheckZ(stringPos, i - stringPos, stringLength));
		...
		ENSURES_B(LOOP_BOUND_OK);
		ENSURES_B(rangeCheck(stringPos, i - stringPos, stringLength));
	...
	IN_LENGTH_ATTRIBUTE const int stringLength)
{
	...
	*dnComponentListPtrPtr = dnComponentPtr;
	LOOP_LARGE_CHECKINC(dnComponentPtr != NULL,
		dnComponentPtr = dnComponentPtr->next)
	{
	...
	IN_OPT const DN_PTR *dnComponentList)
{
	...
	/* Find the end of the DN string.  We have to print the RDNs backwards
	   ...
	   represented */
	LOOP_MED_CHECKINC(dnComponentPtr->next != NULL,
		dnComponentPtr = dnComponentPtr->next)
	{
	...
	LOOP_MED_CHECK(dnComponentPtr != NULL)
	{
		const DN_COMPONENT *dnComponentCursor;
		int LOOP_ITERATOR_ALT;

		/* Find the start of the RDN */
		LOOP_MED_CHECKINC_ALT(dnComponentPtr->prev != NULL && \
		...
		LOOP_MED_INITCHECK_ALT(status = CRYPT_OK,
