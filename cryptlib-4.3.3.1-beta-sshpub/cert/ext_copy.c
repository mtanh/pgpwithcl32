...
static int copyAttributeField( OUT_PTR_COND ATTRIBUTE_LIST **destAttributeField,
	...
	*destAttributeField = NULL;
	...
	if( checkAttributeProperty( srcAttributeField, 
		...
		/* For blob-type attributes the OID is stored at the end of the
		   ...
		   the copy */
		newElement->oid = newElement->storage + newElement->valueLength;
		}
...
static int copyAttribute( INOUT_PTR ATTRIBUTE_LIST **destListHeadPtr,
	...
	LOOP_LARGE( insertPoint = *destListHeadPtr, 
				insertPoint != NULL && \
...
int copyAttributes( INOUT ATTRIBUTE_PTR **destHeadPtr,
	...
	*errorType = CRYPT_ERRTYPE_NONE;
	...
	if( *destHeadPtr != NULL )
		...
		LOOP_LARGE( attributeListCursor = srcListPtr, 
	...
	if( srcListPtr != NULL )
		...
		LOOP_LARGE( insertPoint = *destHeadPtr, 
