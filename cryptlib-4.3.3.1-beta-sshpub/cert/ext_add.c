...
#ifdef USE_RPKI
	LOOP_EXT_INITCHECK( insertPoint = attributeListPtr, 
						insertPoint != NULL && \
							insertPoint->fieldID != CRYPT_ATTRIBUTE_NONE && \
							insertPoint->fieldID <= fieldID,
						FAILSAFE_ITERATIONS_LARGE * 10 )
#else
	LOOP_LARGE_INITCHECK( insertPoint = attributeListPtr, 
						  insertPoint != NULL && \
							insertPoint->fieldID != CRYPT_ATTRIBUTE_NONE && \
							insertPoint->fieldID <= fieldID )
#endif /* USE_RPKI */
...
assert( ( flags & ~( ATTR_FLAG_NONE | ATTR_FLAG_IGNORED | ATTR_FLAG_BLOB | ATTR_FLAG_MULTIVALUED ) ) == 0 );
	...
	if( *listHeadPtr != NULL )
		...
		LOOP_LARGE( insertPoint = *listHeadPtr, insertPoint != NULL, 
					insertPoint = insertPoint->next )
			{
...
int deleteCompositeAttributeField( INOUT ATTRIBUTE_PTR **attributePtr,
	...
	LOOP_MED_INITCHECK( attributeListCursor = listItem, 
...
int deleteCompleteAttribute( INOUT ATTRIBUTE_PTR **attributeListPtr,
	...
	LOOP_LARGE( attributeListCursor = *attributeListPtr, 
				attributeListCursor != NULL && \
					attributeListCursor->attributeID != attributeID,
				attributeListCursor = attributeListCursor->next );
	ENSURES( LOOP_BOUND_OK );
	ENSURES( attributeListCursor != NULL && \
			 ( attributeListCursor->next == NULL || \
...
#ifdef USE_RPKI
	LOOP_EXT_CHECK( attributeListCursor != NULL, 
					FAILSAFE_ITERATIONS_LARGE * 10 )
#else
	LOOP_LARGE_CHECK( attributeListCursor != NULL )
#endif /* USE_RPKI */
