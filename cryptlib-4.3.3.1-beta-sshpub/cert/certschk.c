...
int checkCertDetails( INOUT CERT_INFO *subjectCertInfoPtr,
	...
	REQUIRES( issuerCertInfoPtr == NULL || \
			  sanityCheckCert( subjectCertInfoPtr ) );
...
/* Check the validity of a certificate object against another object, 
   ...
   verification:
	...
	Leaf		Chain		 N		L: A			Enforced by kernel
									R: A - B - C

	Leaf		Chain - Lf.	 N	(Y?)L: A			Enforced by kernel
   ...
   kernel), it's not even possible to poll users on what they'd expected to 
...
int checkCertValidity( INOUT CERT_INFO *certInfoPtr, 
	...
	/* Beyond this point we're checking a certificate, CRL, or signed RTCS 
	   or OCSP response with either another certificate or a raw public key.
	   Certificate chains have been checked above and PKCS #10/CRMF requests 
	   are self-signed so typically they're checked via the 
	   iSigCheckObject == CRYPT_UNUSED option aboave, however they can also
	   be checked by explicitly supplying a check object */
	REQUIRES( certInfoPtr->type == CRYPT_CERTTYPE_CERTIFICATE || \
			  certInfoPtr->type == CRYPT_CERTTYPE_ATTRIBUTE_CERT || \
	...
	/* If we're the check object is a certificate chain, select the leaf
	...
	if( checkObjectType == OBJECT_TYPE_CERTIFICATE )
		...
		if( cryptStatusError( status ) )
			{
			if( checkObjectSubtype == SUBTYPE_CERT_CERTCHAIN )
				{
