...
static BOOLEAN sanityCheckSelectionInfo( const CERT_INFO *certInfoPtr )
	...
	if( currentSelection->dnPtr == NULL && currentSelection->dnInExtension )
		{
		DEBUG_PRINT(( "sanityCheckSelectionInfo: DN selection" ));
		return( FALSE );
		}
	...
	if( currentSelection->dnPtr != NULL && \
		...
		{
		DEBUG_PRINT(( "sanityCheckSelectionInfo: Selected DN" ));
		return( FALSE );
		}
	...
	if( isGeneralNameSelected( certInfoPtr ) && \
		...
		{
		DEBUG_PRINT(( "sanityCheckSelectionInfo: GeneralName selection" ));
		return( FALSE );
		}
	...
	if( currentSelection->dnComponent == CRYPT_ATTRIBUTE_NONE )
		...
		if( currentSelection->dnComponentCount != 0 )
			{
			DEBUG_PRINT(( "sanityCheckSelectionInfo: Spurious DN component count" ));
			return( FALSE );
			}
		}
	else
		...
		if( currentSelection->dnComponentCount < 0 || \
			...
			{
			DEBUG_PRINT(( "sanityCheckSelectionInfo: DN component count" ));
			return( FALSE );
			}
		}
...
int selectDN( INOUT CERT_INFO *certInfoPtr, 
	...
	/* If there's a DN already selected and there's no GeneralName selection
	   ...
	   should create the in the pending GeneralName rather than using the
...
static int setCursorValInfo( INOUT CERT_INFO *certInfoPtr, 
	...
	switch( cursorMoveType )
		...
		case CRYPT_CURSOR_PREVIOUS:
			...
			LOOP_LARGE_CHECKINC( valInfo != NULL && \
		...
		case CRYPT_CURSOR_LAST:
			...
			LOOP_LARGE_CHECKINC( valInfo->next != NULL,
								 valInfo = valInfo->next );
			ENSURES( LOOP_BOUND_OK );
			certValInfo->currentValidity = valInfo;
			break;
	...
	switch( cursorMoveType )
		...
		case CRYPT_CURSOR_PREVIOUS:
			...
			/* Find the previous element in the list.  We use  
			   ...
			   enormous */
			LOOP_MAX_CHECKINC( revInfo != NULL && \
		...
		case CRYPT_CURSOR_LAST:
			...
			LOOP_MAX_CHECKINC( revInfo->next != NULL,
							   revInfo = revInfo->next );
			ENSURES( LOOP_BOUND_OK );
			certRevInfo->currentRevocation = revInfo;
			break;
