...
/* The processUserAuth() function has multiple possible return values,
   ...
   subtypes are:
	...
	OK_SPECIAL + uAI = USERAUTH_NOOP_2: Additional no-op read for a client
		partial pubkey auth without a signature, requiring anyother round
...
/* Read the public key value that'll be used for publickey authentication
   and verify the authentication data created with it */
	...
	IN_LENGTH_TEXT const int userNameLength)
{
	...
	/* Make sure that this key is valid for authentication purposes, in
	   other words that it's present in the authentication keyset and that
	   the name in the certificate matches the supplied user name */
	...
	if (cryptStatusError(status))
	{
		retExt( CRYPT_ERROR_PERMISSION,
			(CRYPT_ERROR_PERMISSION, SESSION_ERRINFO,
				"Client public key with ID '%02X %02X %02X %02X ... "
				"%02X %02X %02X %02X' is not trusted for authentication "
				"purposes",
				keyID[0], keyID[1], keyID[2], keyID[3],
				keyID[msgData.length - 4], keyID[msgData.length - 3],
				  keyID[ msgData.length - 2 ], keyID[ msgData.length - 1 ] ) );
	}
	...
	*userAuthInfo = USERAUTH_ERROR;
	...
	if (methodInfoPtr->type == METHOD_PUBKEY)
	{
		/* In yet another variant of SSH's Lucy-and-Charlie-Brown
		   authentication process, for pubkey auth the caller can supply a
