...
static int processServerKeyex( INOUT SESSION_INFO *sessionInfoPtr,
	...
	const BOOLEAN isPSK = ( handshakeInfo->authAlgo == CRYPT_ALGO_NONE ) ? \
						  TRUE : FALSE;
	const BOOLEAN isTLSLTS = ( sessionInfoPtr->protocolFlags & SSL_PFLAG_TLS12LTS ) ? \
							 TRUE : FALSE;
	...
	if( status == OK_SPECIAL )
		...
		if( sessionIDsent && \
			...
			!memcmp( handshakeInfo->sessionID, sentSessionID,
					 sentSessionIDlength ) )
			{
			DEBUG_PRINT(( "Server negotiated session resumption.\n" ));
			resumeSession = TRUE;
			}
		status = CRYPT_OK;
		}
...
static int exchangeClientKeys( INOUT SESSION_INFO *sessionInfoPtr,
	...
	if( needClientCert )
		...
		if( sessionInfoPtr->privateKey == CRYPT_ERROR )
			...
			if( sessionInfoPtr->version == SSL_MINOR_VERSION_SSL )
				{
				static const BYTE FAR_DATA noCertAlertSSLTemplate[] = {
					SSL_MSG_ALERT,							/* ID */
