...
static const void *getAttrFunction( IN_OPT TYPECAST( ATTRIBUTE_LIST * ) \
	...
	if( !subGroupMove && attrGetType != ATTR_CURRENT )
		...
		attributeListPtr = ( attrGetType == ATTR_PREV ) ? \
							 DATAPTR_GET( attributeListPtr->prev ) : \
...
int setSessionAttributeCursor( INOUT SESSION_INFO *sessionInfoPtr,
	...
	if( position == CRYPT_CURSOR_FIRST || position == CRYPT_CURSOR_LAST )
		...
		/* If there are no attributes present let the caller know */
		if( attributeListCursor == NULL )
			return( CRYPT_ERROR_NOTFOUND );
		}
	else
...
static int addInfo( INOUT SESSION_INFO *sessionInfoPtr,
	...
	insertDoubleListElementsSafe( sessionInfoPtr->attributeList, insertPoint, 
								  newElement, newElement, ATTRIBUTE_LIST );
...
int deleteSessionInfo( INOUT SESSION_INFO *sessionInfoPtr,
	...
	deleteDoubleListElementSafe( sessionInfoPtr->attributeList, 
...
void deleteSessionInfoAll( INOUT SESSION_INFO *sessionInfoPtr )
	...
	if( attributeListCursor == NULL )
		{
		REQUIRES_V( DATAPTR_GET( sessionInfoPtr->attributeListCurrent ) == NULL );
		return;
		}
	...
	ENSURES_V( DATAPTR_GET( sessionInfoPtr->attributeList ) == NULL );
	ENSURES_V( DATAPTR_GET( sessionInfoPtr->attributeListCurrent ) == NULL );
	}
#endif /* USE_SESSIONS */
