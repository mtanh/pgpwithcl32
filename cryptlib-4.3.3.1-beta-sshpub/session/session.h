...
/* Session information flags.  These are:
	...
	SESSION_CACHEDINFO: The session has been recreated from cached 
...
#define SESSION_CACHEDINFO			0x0400	/* Session established from cached info */
#define SESSION_MAX					0x07FF	/* Maximum possible flag value */
...
typedef struct AL {
	...
	DATAPTR_DECLARE( struct AL *, prev );
	DATAPTR_DECLARE( struct AL *, next );/* Prev, next item in the list */
...
typedef struct SI {
	...
	/* Session-related attributes, a current-position cursor in the 
	   attribute list */
	DATAPTR_DECLARE( ATTRIBUTE_LIST *, attributeList ); 
	DATAPTR_DECLARE( ATTRIBUTE_LIST *, attributeListCurrent );
