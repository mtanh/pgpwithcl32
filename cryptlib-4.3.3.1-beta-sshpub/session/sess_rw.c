...
BOOLEAN sanityCheckSessionRead( const SESSION_INFO *sessionInfoPtr )
	...
	if( sessionInfoPtr->receiveBufSize < MIN_BUFFER_SIZE || \
		...
		{
		DEBUG_PRINT(( "sanityCheckSessionRead: Receive buffer size" ));
		return( FALSE );
		}
	if( sessionInfoPtr->type == CRYPT_SESSION_SSH || \
		...
		if( sessionInfoPtr->maxPacketSize < PACKET_SIZE_MIN || \
			...
			{
			DEBUG_PRINT(( "sanityCheckSessionRead: Max packet size" ));
			return( FALSE );
			}
		}
	else
		...
		if( sessionInfoPtr->maxPacketSize != 0 )
			{
			DEBUG_PRINT(( "sanityCheckSessionRead: Spurious max packet size" ));
			return( FALSE );
			}
		}
	...
	if( sessionInfoPtr->receiveBufEnd < 0 || \
		...
		{
		DEBUG_PRINT(( "sanityCheckSessionRead: Receive buffer info" ));
		return( FALSE );
		}
	if( sessionInfoPtr->receiveBufStartOfs < 0 || \
		...
		{
		DEBUG_PRINT(( "sanityCheckSessionRead: Receive buffer packet info" ));
		return( FALSE );
		}
	...
	if( pendingPacketLength < 0 || \
		...
		{
		DEBUG_PRINT(( "sanityCheckSessionRead: Pending packet info" ));
		return( FALSE );
		}
	if( ( sessionInfoPtr->receiveBufEnd - \
		...
		pendingPacketLength )
		{
		DEBUG_PRINT(( "sanityCheckSessionRead: Pending packet calculation" ));
		return( FALSE );
		}
	...
	if( sessionInfoPtr->partialHeaderRemaining < 0 || \
		...
		{
		DEBUG_PRINT(( "sanityCheckSessionRead: Partial header info" ));
...
BOOLEAN sanityCheckSessionWrite( const SESSION_INFO *sessionInfoPtr )
	...
	if( sessionInfoPtr->type != CRYPT_SESSION_SSH && \
		...
		if( sessionInfoPtr->sendBuffer != NULL || \
			...
			{
			DEBUG_PRINT(( "sanityCheckSessionWrite: Spurious send buffer" ));
			return( FALSE );
			}
	...
	if( sessionInfoPtr->sendBufSize < MIN_BUFFER_SIZE || \
		...
		{
		DEBUG_PRINT(( "sanityCheckSessionWrite: Send buffer size" ));
		return( FALSE );
		}
	if( sessionInfoPtr->sendBufStartOfs < 0 || \
		sessionInfoPtr->sendBufStartOfs > FIXED_HEADER_MAX )
		{
		DEBUG_PRINT(( "sanityCheckSessionWrite: Send buffer offset" ));
		return( FALSE );
		}
	if( sessionInfoPtr->partialWrite != TRUE && \
		sessionInfoPtr->partialWrite != FALSE )
		{
		DEBUG_PRINT(( "sanityCheckSessionWrite: Partial write" ));
		return( FALSE );
		}
	...
	if( sessionInfoPtr->sendBufPos < sessionInfoPtr->sendBufStartOfs || \
		...
		{
		DEBUG_PRINT(( "sanityCheckSessionWrite: Send buffer info" ));
		return( FALSE );
		}
	if( sessionInfoPtr->sendBufPartialBufPos < 0 || \
		sessionInfoPtr->sendBufPartialBufPos >= sessionInfoPtr->sendBufPos )
		{
		DEBUG_PRINT(( "sanityCheckSessionWrite: Send buffer partial position" ));
		return( FALSE );
		}
	if( !sessionInfoPtr->partialWrite )
		...
		if( sessionInfoPtr->sendBufPos > sessionInfoPtr->sendBufStartOfs + \
			...
			{
			DEBUG_PRINT(( "sanityCheckSessionWrite: Write position" ));
			return( FALSE );
			}
		}
	else
		...
		if( sessionInfoPtr->sendBufPartialBufPos >= sessionInfoPtr->sendBufPos )
			{
			DEBUG_PRINT(( "sanityCheckSessionWrite: Partial write position" ));
			return( FALSE );
			}
		}
