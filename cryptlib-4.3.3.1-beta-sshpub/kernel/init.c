...
BOOLEAN sanityCheckObject( const OBJECT_INFO *objectInfoPtr )
	...
	if( objectInfoPtr->type <= OBJECT_TYPE_NONE || \
		...
		{
		DEBUG_PRINT(( "sanityCheckObject: Type" ));
		return( FALSE );
		}
	if( objectInfoPtr->subType <= SUBTYPE_NONE || \
		objectInfoPtr->subType > SUBTYPE_LAST )
		{
		DEBUG_PRINT(( "sanityCheckObject: Subtype" ));
		return( FALSE );
		}
	if( DATAPTR_GET( objectInfoPtr->objectPtr ) == NULL )
		{
		DEBUG_PRINT(( "sanityCheckObject: Object info" ));
		return( FALSE );
		}
	if( objectInfoPtr->type == OBJECT_TYPE_CONTEXT && \
		...
		if( objectInfoPtr->objectSize < 1024 || \
			...
			{
			DEBUG_PRINT(( "sanityCheckObject: PKC object size" ));
			return( FALSE );
			}
		}
	else
		...
		if( objectInfoPtr->objectSize < 32 || \
			...
			{
			DEBUG_PRINT(( "sanityCheckObject: Object size" ));
			return( FALSE );
			}
		}
	...
	if( !DATAPTR_ISVALID( objectInfoPtr->objectPtr ) )
		{
		DEBUG_PRINT(( "sanityCheckObject: Object pointer" ));
		return( FALSE );
		}
	if( !FNPTR_ISVALID( objectInfoPtr->messageFunction ) )
		{
		DEBUG_PRINT(( "sanityCheckObject: Message function" ));
		return( FALSE );
		}

	/* Check object properties */
	if( objectInfoPtr->flags < OBJECT_FLAG_NONE || \
		objectInfoPtr->flags > OBJECT_FLAG_MAX )
		{
		DEBUG_PRINT(( "sanityCheckObject: Flags" ));
		return( FALSE );
		}
	if( objectInfoPtr->type == OBJECT_TYPE_CONTEXT )
		{
		switch( objectInfoPtr->subType )
			...
			case SUBTYPE_CTX_CONV:
				if( objectInfoPtr->actionFlags & \
					...
					{
					DEBUG_PRINT(( "sanityCheckObject: Conventional actions" ));
					return( FALSE );
					}
				break;
			...
			case SUBTYPE_CTX_PKC:
				if( objectInfoPtr->actionFlags & \
						~( MK_ACTION_PERM( MESSAGE_CTX_ENCRYPT, ACTION_PERM_ALL ) | \
						   ...
						   MK_ACTION_PERM( MESSAGE_CTX_GENKEY, ACTION_PERM_ALL ) ) )
					{
					DEBUG_PRINT(( "sanityCheckObject: PKC actions" ));
					return( FALSE );
					}
				break;
			...
			case SUBTYPE_CTX_HASH:
				if( objectInfoPtr->actionFlags & \
					...
					{
					DEBUG_PRINT(( "sanityCheckObject: Hash actions" ));
					return( FALSE );
					}
				break;
			...
			case SUBTYPE_CTX_MAC:
				if( objectInfoPtr->actionFlags & \
						~( MK_ACTION_PERM( MESSAGE_CTX_HASH, ACTION_PERM_ALL ) | \
						   MK_ACTION_PERM( MESSAGE_CTX_GENKEY, ACTION_PERM_ALL ) ) )
					{
					DEBUG_PRINT(( "sanityCheckObject: MAC actions" ));
					return( FALSE );
					}
				break;
			...
			case SUBTYPE_CTX_GENERIC:
				if( ( objectInfoPtr->actionFlags & \
					...
					{
					DEBUG_PRINT(( "sanityCheckObject: Generic actions" ));
					return( FALSE );
					}
				break;
		...
		if( objectInfoPtr->actionFlags != ACTION_PERM_FLAG_NONE )
			{
			DEBUG_PRINT(( "sanityCheckObject: Spurious actions" ));
			return( FALSE );
			}
		}
	if( objectInfoPtr->intRefCount < 0 || \
		objectInfoPtr->intRefCount > MAX_INTLENGTH || \
		objectInfoPtr->extRefCount < 0 || \
		objectInfoPtr->extRefCount > MAX_INTLENGTH )
		{
		DEBUG_PRINT(( "sanityCheckObject: Reference count" ));
		return( FALSE );
		}
	if( objectInfoPtr->lockCount < 0 || \
		objectInfoPtr->lockCount > MAX_INTLENGTH ) 
		{
		DEBUG_PRINT(( "sanityCheckObject: Lock count" ));
		return( FALSE );
		}
	...
	if( ( objectInfoPtr->type == OBJECT_TYPE_DEVICE && \
		...
		if( objectInfoPtr->dependentObject != CRYPT_ERROR || \
			...
			{
			DEBUG_PRINT(( "sanityCheckObject: Spurious dependent objects" ));
			return( FALSE );
			}
		}
	else
		...
		if( objectInfoPtr->owner != DEFAULTUSER_OBJECT_HANDLE && \
			...
			{
			DEBUG_PRINT(( "sanityCheckObject: Owner handle" ));
			return( FALSE );
			}
		if( objectInfoPtr->dependentObject != CRYPT_ERROR && \
			!isHandleRangeValid( objectInfoPtr->dependentObject ) )
			{
			DEBUG_PRINT(( "sanityCheckObject: Dependent object" ));
			return( FALSE );
			}
		if( objectInfoPtr->dependentDevice != CRYPT_ERROR && \
			...
			{
			DEBUG_PRINT(( "sanityCheckObject: Dependent device" ));
			return( FALSE );
			}
		}
...
CHECK_RETVAL \
int krnlWait( IN_RANGE( 1, 1000 ) const int milliSeconds )
	{
	REQUIRES( milliSeconds >= 1 && milliSeconds <= 1000 );
