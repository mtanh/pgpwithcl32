...
static const MESSAGE_ACL FAR_DATA messageParamACLTbl[] = {
...
static const MESSAGE_HANDLING_INFO FAR_DATA messageHandlingInfo[] = {
...
static int enqueueMessage( IN_HANDLE const int objectHandle,
	...
	DATAPTR_SET( messageQueue[ queuePos ].handlingInfoPtr, handlingInfoPtr );
	messageQueue[ queuePos ].message = message;
	DATAPTR_SET( messageQueue[ queuePos ].messageDataPtr, messageDataPtr );
...
static int dequeueMessage( IN_RANGE( 0, MESSAGE_QUEUE_SIZE ) \
	...
	FORALL( i, 0, krnlData->queueEnd,
			DATAPTR_GET( messageQueue[ i ].handlingInfoPtr ) != NULL );
...
static int dispatchMessage( IN_HANDLE const int localObjectHandle,
	...
	REQUIRES( objectPtr != NULL );
			  /* We can't check messageDataPtr because this is NULL for a 
			     number of messages */
...
int krnlSendMessage( IN_HANDLE const int objectHandle, 
	...
	if( !isInUse( localObjectHandle ) && localMessage != MESSAGE_DESTROY )
		...
		DATAPTR_SET( messageQueueData.handlingInfoPtr, handlingInfoPtr ); 
