...
typedef struct {
	...
	const char FAR_DATA *headerString;/* Header string */
...
static const HTTP_HEADER_PARSE_INFO FAR_DATA httpHeaderParseInfo[] = {
...
typedef struct {
	...
	const char FAR_DATA *httpStatusString;	/* String status value */
	BUFFER_FIXED( httpErrorStringLength ) \
	const char FAR_DATA *httpErrorString;	/* Text description of status */
	const int httpErrorStringLength;
	const int status;				/* Equivalent cryptlib status */
	} HTTP_STATUS_INFO;

static const HTTP_STATUS_INFO FAR_DATA httpStatusInfo[] = {
