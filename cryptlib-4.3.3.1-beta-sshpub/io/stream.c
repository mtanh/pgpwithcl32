...
static BOOLEAN sanityCheckStream( const STREAM *stream )
	...
	if( stream->type <= STREAM_TYPE_NONE || \
		stream->type >= STREAM_TYPE_LAST || \
		stream->flags < STREAM_FLAG_NONE || \
		stream->flags > STREAM_FLAG_MAX )
		{
		DEBUG_PRINT(( "sanityCheckStream: General info" ));
		return( FALSE );
		}
	...
	switch( stream->type )
		...
		case STREAM_TYPE_NULL:
			...
			if( stream->flags != 0 )
				{
				DEBUG_PRINT(( "sanityCheckStream: Spurious null stream flags" ));
				return( FALSE );
				}
			...
			if( stream->buffer != NULL || stream->bufSize != 0 )
				{
				DEBUG_PRINT(( "sanityCheckStream: Spurious null stream buffer" ));
				return( FALSE );
				}
			if( stream->bufPos < 0 || stream->bufPos > stream->bufEnd || 
				stream->bufEnd < 0 || stream->bufEnd >= MAX_BUFFER_SIZE )
				{
				DEBUG_PRINT(( "sanityCheckStream: Null stream position" ));
				return( FALSE );
				}
		...
		case STREAM_TYPE_MEMORY:
			...
			if( stream->flags & STREAM_MFLAG_VFILE )
				...
				if( stream->flags & ~( STREAM_FLAG_MASK | STREAM_MFLAG_VFILE | \
									   STREAM_FFLAG_MASK ) )
					{
					DEBUG_PRINT(( "sanityCheckStream: Memory stream flags" ));
					return( FALSE );
					}
				}
			else
				...
				if( stream->flags & ~STREAM_MFLAG_MASK )
					{
					DEBUG_PRINT(( "sanityCheckStream: Spurious memory stream flags" ));
					return( FALSE );
					}
				}
			break;
		...
		case STREAM_TYPE_FILE:
			...
			if( stream->flags & ~STREAM_FFLAG_MASK )
				{
				DEBUG_PRINT(( "sanityCheckStream: Spurious file stream flags" ));
				return( FALSE );
				}
			...
			if( !( stream->flags & STREAM_FFLAG_BUFFERSET ) )
				...
				if( stream->buffer != NULL || stream->bufPos != 0 || \
					...
					{
					DEBUG_PRINT(( "sanityCheckStream: Spurious file buffer" ));
					return( FALSE );
					}
			...
			if( stream->bufCount < 0 || \
				...
				{
				DEBUG_PRINT(( "sanityCheckStream: File stream buffer position" ));
				return( FALSE );
				}
		...
		case STREAM_TYPE_NETWORK:
			...
			if( !DATAPTR_ISVALID( stream->netStream ) )
				{
				DEBUG_PRINT(( "sanityCheckStream: Net stream pointer" ));
				return( FALSE );
				}
			...
			if( stream->buffer == NULL )
				...
				if( stream->bufPos != 0 || stream->bufSize != 0 || \
					stream->bufEnd != 0 )
					{
					DEBUG_PRINT(( "sanityCheckStream: Spurious net stream buffer" ));
					return( FALSE );
					}
				}	
			...
			if( !sanityCheckNetStream( netStream ) )
				{
				DEBUG_PRINT(( "sanityCheckStream: Net stream info" ));
				return( FALSE );
				}
	...
	if( stream->buffer == NULL )
		{
		DEBUG_PRINT(( "sanityCheckStream: Stream buffer" ));
		return( FALSE );
		}
	...
	if( stream->bufPos < 0 || stream->bufPos > stream->bufEnd || \
		...
		{
		DEBUG_PRINT(( "sanityCheckStream: Stream buffer info" ));
		return( FALSE );
		}
	...
	switch( stream->type )
		...
		case STREAM_TYPE_MEMORY:
			...
			if( sIsPseudoHTTPRawStream( stream ) )
				...
				readFunction = FNPTR_GET( netStream->readFunction );
	...
	switch( stream->type )
		...
		case STREAM_TYPE_NETWORK:
			...
			/* Set up the function pointers.  We have to do this after the 
			   netStream check otherwise we'd potentially be dereferencing a 
			   NULL pointer */
			writeFunction = FNPTR_GET( netStream->writeFunction );
