...
static BOOLEAN sanityCheckStreamMem( const STREAM *stream )
	...
	if( stream->type == STREAM_TYPE_NULL )
		...
		if( stream->bufSize != 0 )
			{
			DEBUG_PRINT(( "sanityCheckStreamMem: Spurious null stream buffer" ));
			return( FALSE );
			}
		if( stream->bufPos < 0 || stream->bufPos > stream->bufEnd || 
			stream->bufEnd < 0 || stream->bufEnd >= MAX_BUFFER_SIZE )
			{
			DEBUG_PRINT(( "sanityCheckStreamMem: Null stream position" ));
			return( FALSE );
			}
	...
	if( stream->type != STREAM_TYPE_MEMORY )
		{
		DEBUG_PRINT(( "sanityCheckStreamMem: Stream type" ));
		return( FALSE );
		}
	...
	if( stream->bufPos < 0 || stream->bufPos > stream->bufEnd || \
		...
		{
		DEBUG_PRINT(( "sanityCheckStreamMem: Position" ));
		return( FALSE );
		}
