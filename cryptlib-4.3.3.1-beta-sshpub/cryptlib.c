...
static BOOLEAN sanityCheckBuild( void )
	...
	if( !isValidPointer( data ) || !isValidPointer( preInitFunctions ) )
		{
		DEBUG_PRINT(( "isValidPointer() macro reports data or code pointer "
...
int initCryptlib( void )
	...
	DEBUG_PRINT(( "** Initialising kernel **\n" ));
	...
	/* Perform OS-specific additional initialisation inside the kernel init 
	   lock */
	DEBUG_PRINT(( "** Initialising system variables **\n" ));
	status = initSysVars();
	if( cryptStatusError( status ) )
		{
	...
	DEBUG_PRINT(( "** Running pre-init actions **\n" ));
	...
	if( cryptStatusOK( status ) )
		{
		initLevel = 1;
		DEBUG_PRINT(( "** Running init actions **\n" ));
	...
	if( cryptStatusOK( status ) )
		...
		/* Perform the final init phase asynchronously or synchronously 
		   ...
		   exist at this time */
		DEBUG_PRINT(( "** Running async init actions **\n" ));
#ifdef USE_THREAD_FUNCTIONS
	...
	DEBUG_PRINT(( "** Completing kernel init **\n" ));
	krnlCompleteInit();
