...
CHECK_RETVAL_BOOL \
static BOOLEAN sanityCheckObjectHandle( const CRYPT_HANDLE cryptHandle )
	{
...
BOOLEAN sanityCheckSession( const SESSION_INFO *sessionInfoPtr )
	...
	if( sessionInfoPtr->type <= CRYPT_SESSION_NONE || \
		...
		sessionInfoPtr->flags < SESSION_NONE || \
		sessionInfoPtr->flags > SESSION_MAX ) 
		{
		DEBUG_PRINT(( "sanityCheckSession: General info" ));
		return( FALSE );
		}
	if( sessionInfoPtr->cryptAlgo != CRYPT_ALGO_NONE && \
		!isConvAlgo( sessionInfoPtr->cryptAlgo ) )
		{
		DEBUG_PRINT(( "sanityCheckSession: Crypt algorithm" ));
		return( FALSE );
		}
	if( sessionInfoPtr->integrityAlgo != CRYPT_ALGO_NONE && \
		...
		{
		DEBUG_PRINT(( "sanityCheckSession: Integrity algorithm" ));
		return( FALSE );
		}
	if( sessionInfoPtr->authResponse < AUTHRESPONSE_NONE || \
		sessionInfoPtr->authResponse >= AUTHRESPONSE_LAST )
		{
		DEBUG_PRINT(( "sanityCheckSession: Auth response" ));
		return( FALSE );
		}
	...
	if( !sanityCheckSessionRead( sessionInfoPtr ) || \
		...
		{
		DEBUG_PRINT(( "sanityCheckSession: Buffer information" ));
		return( FALSE );
		}

	/* Check crypto information */
	if( !sanityCheckObjectHandle( sessionInfoPtr->iKeyexCryptContext ) || \
		!sanityCheckObjectHandle( sessionInfoPtr->iKeyexAuthContext ) || \
		!sanityCheckObjectHandle( sessionInfoPtr->iCryptInContext ) || \
		!sanityCheckObjectHandle( sessionInfoPtr->iCryptOutContext ) || \
		!sanityCheckObjectHandle( sessionInfoPtr->iAuthInContext ) || \
		!sanityCheckObjectHandle( sessionInfoPtr->iAuthOutContext ) || \
		!sanityCheckObjectHandle( sessionInfoPtr->iCertRequest ) || \
		!sanityCheckObjectHandle( sessionInfoPtr->iCertResponse ) || \
		!sanityCheckObjectHandle( sessionInfoPtr->privateKey ) || \
		!sanityCheckObjectHandle( sessionInfoPtr->cryptKeyset ) || \
		!sanityCheckObjectHandle( sessionInfoPtr->privKeyset ) )
		{
		DEBUG_PRINT(( "sanityCheckSession: Crypto object handles" ));
		return( FALSE );
		}
	if( sessionInfoPtr->cryptBlocksize < 0 || \
		...
		{
		DEBUG_PRINT(( "sanityCheckSession: Crypt/auth block size" ));
		return( FALSE );
		}

	/* Check network information */
	if( !sanityCheckObjectHandle( sessionInfoPtr->transportSession ) )
		{
		DEBUG_PRINT(( "sanityCheckSession: Transport session handle" ));
		return( FALSE );
		}
	if( sessionInfoPtr->flags & ( SESSION_PARTIALOPEN | SESSION_ISOPEN ) )
		{
		...
		if( sessionInfoPtr->readTimeout < 0 || \
			...
			{
			DEBUG_PRINT(( "sanityCheckSession: Timeouts" ));
			return( FALSE );
			}
		}

	/* Check function pointers */
	if( FNPTR_GET( sessionInfoPtr->shutdownFunction ) == NULL || \
		FNPTR_GET( sessionInfoPtr->connectFunction ) == NULL || \
		FNPTR_GET( sessionInfoPtr->transactFunction ) == NULL )
		{
		DEBUG_PRINT(( "sanityCheckSession: General functions" ));
		return( FALSE );
		}
	if( sessionInfoPtr->protocolInfo->isReqResp )
		{
		if( FNPTR_GET( sessionInfoPtr->readHeaderFunction ) != NULL || \
			FNPTR_GET( sessionInfoPtr->processBodyFunction ) != NULL || \
			FNPTR_GET( sessionInfoPtr->preparePacketFunction ) != NULL )
			{
			DEBUG_PRINT(( "sanityCheckSession: Spurious data transfer functions" ));
			return( FALSE );
			}
		}
	else
		...
		if( FNPTR_GET( sessionInfoPtr->readHeaderFunction ) == NULL || \
			...
			FNPTR_GET( sessionInfoPtr->preparePacketFunction ) == NULL )
			{
			DEBUG_PRINT(( "sanityCheckSession: Data transfer functions" ));
			return( FALSE );
			}
		}
	...
	if( !isHandleRangeValid( sessionInfoPtr->objectHandle ) || \
		...
		{
		DEBUG_PRINT(( "sanityCheckSession: Object handles" ));
		return( FALSE );
		}
...
static int sessionMessageFunction( INOUT TYPECAST( CONTEXT_INFO * ) \
	...
	if( message == MESSAGE_DESTROY )
		...
		if( DATAPTR_GET( sessionInfoPtr->attributeList ) != NULL )
	...
	*sessionInfoPtrPtr = sessionInfoPtr;
	...
	if( protocolInfoPtr->isReqResp )
		...
		sessionInfoPtr->sendBufStartOfs = sessionInfoPtr->receiveBufStartOfs = \
				sessionInfoPtr->sendBufPos = protocolInfoPtr->sendBufStartOfs;
